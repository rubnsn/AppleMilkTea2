...
> Config: `DCsConfig.potionIDAbsHeal` (`DCsConfig.java:9`, 0-127 EndlessIDs)
> Field: `DCsAppleMilk.absHeal` (`DCsAppleMilk.java:xxx`)
> Color: `0xFF32C0` / 良悪: `良`
> Manager: `AMTPotionManager.manager.getPotion("absHeal")` / `PotionGetter.java:1`
...
- **登録**: `if(Potion.potionTypes[DCsConfig.potionIDAbsHeal]==null && DCsConfig.potionIDAbsHeal<128){ DCsAppleMilk.absHeal = new PotionReflex(DCsConfig.potionIDAbsHeal, false, 0xFF32C0, x,y).setPotionName("DCs.potion.absHeal"); }` (`MaterialRegister.java:414`)
...
- [ ] `DCsConfig.potionIDAbsHeal` 整数ID -> 削除、`ResourceLocation` へ
