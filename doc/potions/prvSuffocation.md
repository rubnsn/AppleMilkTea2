...
> Source: `src/main/java/mods/defeatedcrow/potion/PotionProtectionEX.java:1`
> Registry: `Potion.potionTypes[ID]` (`MaterialRegister.java:413` `addPotion()`)
> Config: `DCsConfig.potionIDPrvSuffocation` (`DCsConfig.java:9`, 0-127 EndlessIDs)
> Field: `DCsAppleMilk.prvSuffocation` (`DCsAppleMilk.java:xxx`)
> Color: `0xC064FF` / 良悪: `良`
> Manager: `AMTPotionManager.manager.getPotion("prvSuffocation")` / `PotionGetter.java:1`

## 概要
窒息耐性。inWall由来。setProtectSuffocation()。

## 登録情報
- **クラス**: `PotionProtectionEX extends PotionBaseAMT` (`src/main/java/mods/defeatedcrow/potion/PotionProtectionEX.java:1`)
- **登録**: `if(Potion.potionTypes[DCsConfig.potionIDPrvSuffocation]==null && DCsConfig.potionIDPrvSuffocation<128){ DCsAppleMilk.prvSuffocation = new PotionProtectionEX(DCsConfig.potionIDPrvSuffocation, false, 0xC064FF, x,y).setPotionName("DCs.potion.prvSuffocation"); }` (`MaterialRegister.java:414`)
- **失敗時**: `succeedAddPotion=false`
- **色**: `0xC064FF`

## 継承・インターフェース
- 継承: `PotionProtectionEX extends PotionBaseAMT`
- 基底: `PotionBaseAMT` / `PotionImmunityBase` / `PotionReflexBase` / `PotionLivingBase`

## プロパティ / 効果
- **isBadEffect**: `false` (良)
- **liquidColor**: `0xC064FF`
...
- [ ] `DCsConfig.potionIDPrvSuffocation` 整数ID -> 削除、`ResourceLocation` へ
...
> 自動生成: `src/main/java/mods/defeatedcrow/potion/PotionProtectionEX.java:1` / `MaterialRegister.java:413`
> 最終更新: 2026-08-24