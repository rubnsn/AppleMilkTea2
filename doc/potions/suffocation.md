...
> Source: `src/main/java/mods/defeatedcrow/potion/PotionSuffocation.java:1`
> Registry: `Potion.potionTypes[ID]` (`MaterialRegister.java:413` `addPotion()`)
> Config: `DCsConfig.potionIDSuffocation` (`DCsConfig.java:9`, 0-127 EndlessIDs)
> Field: `DCsAppleMilk.suffocation` (`DCsAppleMilk.java:xxx`)
> Color: `0x161616` / 良悪: `悪`
> Manager: `AMTPotionManager.manager.getPotion("suffocation")` / `PotionGetter.java:1`

## 概要
窒息。持続ダメージ。悪。

## 登録情報
- **クラス**: `PotionSuffocation extends PotionBaseAMT` (`src/main/java/mods/defeatedcrow/potion/PotionSuffocation.java:1`)
- **登録**: `if(Potion.potionTypes[DCsConfig.potionIDSuffocation]==null && DCsConfig.potionIDSuffocation<128){ DCsAppleMilk.suffocation = new PotionSuffocation(DCsConfig.potionIDSuffocation, true, 0x161616, x,y).setPotionName("DCs.potion.suffocation"); }` (`MaterialRegister.java:414`)
- **失敗時**: `succeedAddPotion=false`
- **色**: `0x161616`

## 継承・インターフェース
- 継承: `PotionSuffocation extends PotionBaseAMT`
- 基底: `PotionBaseAMT` / `PotionImmunityBase` / `PotionReflexBase` / `PotionLivingBase`

## プロパティ / 効果
- **isBadEffect**: `true` (悪)
- **liquidColor**: `0x161616`
...
- [ ] `DCsConfig.potionIDSuffocation` 整数ID -> 削除、`ResourceLocation` へ
...
> 自動生成: `src/main/java/mods/defeatedcrow/potion/PotionSuffocation.java:1` / `MaterialRegister.java:413`
> 最終更新: 2026-08-24