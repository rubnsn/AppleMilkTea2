...
> Config: `DCsConfig.potionIDAbsEXP` (`DCsConfig.java:9`, 0-127 EndlessIDs)
> Field: `DCsAppleMilk.absEXP` (`DCsAppleMilk.java:xxx`)
> Color: `0x00FF64` / 良悪: `良`
> Manager: `AMTPotionManager.manager.getPotion("absEXP")` / `PotionGetter.java:1`
...
- **登録**: `if(Potion.potionTypes[DCsConfig.potionIDAbsEXP]==null && DCsConfig.potionIDAbsEXP<128){ DCsAppleMilk.absEXP = new PotionReflex(DCsConfig.potionIDAbsEXP, false, 0x00FF64, x,y).setPotionName("DCs.potion.absEXP"); }` (`MaterialRegister.java:414`)
...
- [ ] `DCsConfig.potionIDAbsEXP` 整数ID -> 削除、`ResourceLocation` へ
