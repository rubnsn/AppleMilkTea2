...
> Config: `DCsConfig.potionIDPrvProjectile` (`DCsConfig.java:9`, 0-127 EndlessIDs)
> Field: `DCsAppleMilk.prvProjectile` (`DCsAppleMilk.java:xxx`)
> Color: `0x996420` / 良悪: `良`
> Manager: `AMTPotionManager.manager.getPotion("prvProjectile")` / `PotionGetter.java:1`
...
- **登録**: `if(Potion.potionTypes[DCsConfig.potionIDPrvProjectile]==null && DCsConfig.potionIDPrvProjectile<128){ DCsAppleMilk.prvProjectile = new PotionProtectionEX(DCsConfig.potionIDPrvProjectile, false, 0x996420, x,y).setPotionName("DCs.potion.prvProjectile"); }` (`MaterialRegister.java:414`)
...
- [ ] `DCsConfig.potionIDPrvProjectile` 整数ID -> 削除、`ResourceLocation` へ
