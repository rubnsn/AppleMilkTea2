...
> Source: `src/main/java/mods/defeatedcrow/potion/PotionImmunity.java:1`
> Registry: `Potion.potionTypes[ID]` (`MaterialRegister.java:413` `addPotion()`)
> Config: `DCsConfig.potionIDImmunity` (`DCsConfig.java:9`, 0-127 EndlessIDs)
> Field: `DCsAppleMilk.Immunization` (`DCsAppleMilk.java:xxx`)
> Color: `0xFEC0C0` / 良悪: `良`
> Manager: `AMTPotionManager.manager.getPotion("Immunization")` / `PotionGetter.java:1`

## 概要
免疫。デバフ無効化。amp0:空腹,1:毒/衰弱,2:混乱/盲目/窒息,3:採掘低下等。DCsLivingEventで毎tick除去。

## 登録情報
- **クラス**: `PotionImmunity extends PotionLivingBase` (`src/main/java/mods/defeatedcrow/potion/PotionImmunity.java:1`)
- **登録**: `if(Potion.potionTypes[DCsConfig.potionIDImmunity]==null && DCsConfig.potionIDImmunity<128){ DCsAppleMilk.Immunization = new PotionImmunity(DCsConfig.potionIDImmunity, false, 0xFEC0C0, x,y).setPotionName("DCs.potion.Immunization"); }` (`MaterialRegister.java:414`)
- **失敗時**: `succeedAddPotion=false`
- **色**: `0xFEC0C0`

## 継承・インターフェース
- 継承: `PotionImmunity extends PotionLivingBase`
- 基底: `PotionBaseAMT` / `PotionImmunityBase` / `PotionReflexBase` / `PotionLivingBase`

## プロパティ / 効果
- **isBadEffect**: `false` (良)
- **liquidColor**: `0xFEC0C0`
...
- [ ] `DCsConfig.potionIDImmunity` 整数ID -> 削除、`ResourceLocation` へ
...
> 自動生成: `src/main/java/mods/defeatedcrow/potion/PotionImmunity.java:1` / `MaterialRegister.java:413`
> 最終更新: 2026-08-24