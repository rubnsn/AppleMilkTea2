...
> Config: `DCsConfig.potionIDPrvExplode` (`DCsConfig.java:9`, 0-127 EndlessIDs)
> Field: `DCsAppleMilk.prvExplode` (`DCsAppleMilk.java:xxx`)
> Color: `0x646464` / 良悪: `良`
> Manager: `AMTPotionManager.manager.getPotion("prvExplode")` / `PotionGetter.java:1`

## 概要
爆発耐性。anvil由来。DCsHurtEventで軽減。

## 登録情報
- **クラス**: `PotionProtectionEX extends PotionBaseAMT` (`src/main/java/mods/defeatedcrow/potion/PotionProtectionEX.java:1`)
- **登録**: `if(Potion.potionTypes[DCsConfig.potionIDPrvExplode]==null && DCsConfig.potionIDPrvExplode<128){ DCsAppleMilk.prvExplode = new PotionProtectionEX(DCsConfig.potionIDPrvExplode, false, 0x646464, x,y).setPotionName("DCs.potion.prvExplode"); }` (`MaterialRegister.java:414`)
...
- [ ] `DCsConfig.potionIDPrvExplode` 整数ID -> 削除、`ResourceLocation` へ
