...
> Source: `src/main/java/mods/defeatedcrow/potion/PotionConfinement.java:1`
> Registry: `Potion.potionTypes[ID]` (`MaterialRegister.java:413` `addPotion()`)
> Config: `DCsConfig.potionIDConfinement` (`DCsConfig.java:9`, 0-127 EndlessIDs)
> Field: `DCsAppleMilk.confinement` (`DCsAppleMilk.java:xxx`)
> Color: `0x646464` / 良悪: `悪`
> Manager: `AMTPotionManager.manager.getPotion("confinement")` / `PotionGetter.java:1`

## 概要
束縛。移動制限。悪。DCsLivingEventで減速。

## 登録情報
- **クラス**: `PotionConfinement extends PotionLivingBase` (`src/main/java/mods/defeatedcrow/potion/PotionConfinement.java:1`)
- **登録**: `if(Potion.potionTypes[DCsConfig.potionIDConfinement]==null && DCsConfig.potionIDConfinement<128){ DCsAppleMilk.confinement = new PotionConfinement(DCsConfig.potionIDConfinement, true, 0x646464, x,y).setPotionName("DCs.potion.confinement"); }` (`MaterialRegister.java:414`)
- **失敗時**: `succeedAddPotion=false`
- **色**: `0x646464`

## 継承・インターフェース
- 継承: `PotionConfinement extends PotionLivingBase`
- 基底: `PotionBaseAMT` / `PotionImmunityBase` / `PotionReflexBase` / `PotionLivingBase`

## プロパティ / 効果
- **isBadEffect**: `true` (悪)
- **liquidColor**: `0x646464`
...
- [ ] `DCsConfig.potionIDConfinement` 整数ID -> 削除、`ResourceLocation` へ
...
> 自動生成: `src/main/java/mods/defeatedcrow/potion/PotionConfinement.java:1` / `MaterialRegister.java:413`
> 最終更新: 2026-08-24