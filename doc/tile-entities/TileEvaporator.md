...
> Source: `src/main/java/mods/defeatedcrow/common/tile/appliance/TileEvaporator.java:1`
> Registry: `TileEvaporator` (`CommonProxy.java:70` `GameRegistry.registerTileEntity(TileEvaporator.class, "TileEvaporator")`)
> Block: `TileEvaporator` (`DCsAppleMilk.tileevaporator`)
> GUI: あり (ID `4`)

## 概要
エバポレーター。流体生成。ID4。 `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity` 登録。

## 登録情報
- **クラス**: `TileEvaporator extends MachineBase` (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileEvaporator.java:1`)
- **登録名**: `TileEvaporator` (`GameRegistry.registerTileEntity(TileEvaporator.class, "TileEvaporator")` at `CommonProxy.java:70-117`)
- **対応Block**: `TileEvaporator` の `createNewTileEntity` で生成
...
## Container / GUI 連携
- **Container**: `ContainerEvaporator`
- **Gui**: `GuiEvaporator`
- **ID**: `4` (`CommonProxy.getServerGuiElement` / `getClientGuiElement`)
- **Open**: `player.openGui(mod, ID, world, x,y,z)` -> 1.14+ `NetworkHooks.openGui`

## レンダー
- Renderer: `TileEntityEvaporatorRenderer` (ISBRH/TESR) (`ClientProxy.registerRenderers()`)
...
> 自動生成: `src/main/java/mods/defeatedcrow/common/tile/appliance/TileEvaporator.java:1` / `CommonProxy.java:70`
> 最終更新: 2026-08-24