...
> Source: `src/main/java/mods/defeatedcrow/common/tile/appliance/TileProcessor.java:1`
> Registry: `TileProcessor` (`CommonProxy.java:70` `GameRegistry.registerTileEntity(TileProcessor.class, "TileProcessor")`)
> Block: `TileProcessor` (`DCsAppleMilk.tileprocessor`)
> GUI: あり (ID `3`)

## 概要
プロセッサー。Container/Gui ID3。 `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity` 登録。

## 登録情報
- **クラス**: `TileProcessor extends MachineBase` (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileProcessor.java:1`)
- **登録名**: `TileProcessor` (`GameRegistry.registerTileEntity(TileProcessor.class, "TileProcessor")` at `CommonProxy.java:70-117`)
- **対応Block**: `TileProcessor` の `createNewTileEntity` で生成
...
- **Gui**: `GuiProcessor`
- **ID**: `3` (`CommonProxy.getServerGuiElement` / `getClientGuiElement`)
- **Open**: `player.openGui(mod, ID, world, x,y,z)` -> 1.14+ `NetworkHooks.openGui`

## レンダー
- Renderer: `TileEntityProcessorRenderer` (ISBRH/TESR) (`ClientProxy.registerRenderers()`)
...
> 自動生成: `src/main/java/mods/defeatedcrow/common/tile/appliance/TileProcessor.java:1` / `CommonProxy.java:70`
> 最終更新: 2026-08-24