...
> Source: `src/main/java/mods/defeatedcrow/common/tile/energy/TileHandleEngine.java:1`
> Registry: `TileEHandle` (`CommonProxy.java:70` `GameRegistry.registerTileEntity(TileHandleEngine.class, "TileEHandle")`)
> Block: `-` (`DCsAppleMilk.-`)
> GUI: なし (ID `-`)

## 概要
ハンドル発電機。手動チャージ。 `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity` 登録。

## 登録情報
- **クラス**: `TileHandleEngine extends TileEntity implements IChargeGenerator` (`src/main/java/mods/defeatedcrow/common/tile/energy/TileHandleEngine.java:1`)
- **登録名**: `TileEHandle` (`GameRegistry.registerTileEntity(TileHandleEngine.class, "TileEHandle")` at `CommonProxy.java:70-117`)
- **対応Block**: `-` の `createNewTileEntity` で生成
- **NBTキー**: `Remaining`, `Direction`, `Input` 等 (`writeToNBT`/`readFromNBT`)

## 継承・インターフェース
- 継承: `TileEntity implements IChargeGenerator`
...
## レンダー
- Renderer: `TileEntityHandleEngineRenderer` (ISBRH/TESR) (`ClientProxy.registerRenderers()`)
...
> 自動生成: `src/main/java/mods/defeatedcrow/common/tile/energy/TileHandleEngine.java:1` / `CommonProxy.java:70`
> 最終更新: 2026-08-24