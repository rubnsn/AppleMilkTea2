...
> Source: `src/main/java/mods/defeatedcrow/common/tile/appliance/TileIceMaker.java:1`
> Registry: `TileIceMaker` (`CommonProxy.java:70` `GameRegistry.registerTileEntity(TileIceMaker.class, "TileIceMaker")`)
> Block: `TileIceMaker` (`DCsAppleMilk.tileicemaker`)
> GUI: あり (ID `2`)

## 概要
アイスメーカー。Container/Gui ID2。Charge対応。 `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity` 登録。

## 登録情報
- **クラス**: `TileIceMaker extends MachineBase` (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileIceMaker.java:1`)
- **登録名**: `TileIceMaker` (`GameRegistry.registerTileEntity(TileIceMaker.class, "TileIceMaker")` at `CommonProxy.java:70-117`)
- **対応Block**: `TileIceMaker` の `createNewTileEntity` で生成
...
## Container / GUI 連携
- **Container**: `ContainerIceMaker`
- **Gui**: `GuiIceMaker`
- **ID**: `2` (`CommonProxy.getServerGuiElement` / `getClientGuiElement`)
- **Open**: `player.openGui(mod, ID, world, x,y,z)` -> 1.14+ `NetworkHooks.openGui`

## レンダー
- Renderer: `TileEntityIceMakerRenderer` (ISBRH/TESR) (`ClientProxy.registerRenderers()`)
...
> 自動生成: `src/main/java/mods/defeatedcrow/common/tile/appliance/TileIceMaker.java:1` / `CommonProxy.java:70`
> 最終更新: 2026-08-24