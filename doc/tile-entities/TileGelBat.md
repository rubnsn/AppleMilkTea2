...
> Source: `src/main/java/mods/defeatedcrow/common/tile/energy/TileGelBat.java:1`
> Registry: `TileGelBattery` (`CommonProxy.java:70` `GameRegistry.registerTileEntity(TileGelBat.class, "TileGelBattery")`)
> Block: `-` (`DCsAppleMilk.-`)
> GUI: なし (ID `-`)

## 概要
ゲル電池。 `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity` 登録。

## 登録情報
- **クラス**: `TileGelBat extends TileChargerDevice` (`src/main/java/mods/defeatedcrow/common/tile/energy/TileGelBat.java:1`)
- **登録名**: `TileGelBattery` (`GameRegistry.registerTileEntity(TileGelBat.class, "TileGelBattery")` at `CommonProxy.java:70-117`)
- **対応Block**: `-` の `createNewTileEntity` で生成
- **NBTキー**: `Remaining`, `Direction`, `Input` 等 (`writeToNBT`/`readFromNBT`)

## 継承・インターフェース
- 継承: `TileChargerDevice`
...
## レンダー
- Renderer: `TileEntityGelBatRenderer` (ISBRH/TESR) (`ClientProxy.registerRenderers()`)
...
> 自動生成: `src/main/java/mods/defeatedcrow/common/tile/energy/TileGelBat.java:1` / `CommonProxy.java:70`
> 最終更新: 2026-08-24