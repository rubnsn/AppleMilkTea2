...
> Source: `src/main/java/mods/defeatedcrow/common/tile/appliance/TileMakerNext.java:1`
> Registry: `TileMakerNext` (`CommonProxy.java:70` `GameRegistry.registerTileEntity(TileMakerNext.class, "TileMakerNext")`)
> Block: `TileMakerNext` (`DCsAppleMilk.tilemakernext`)
> GUI: なし (ID `-`)

## 概要
ティーメーカーTile。ITeaMakerでTeaRecipe処理。 `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity` 登録。

## 登録情報
- **クラス**: `TileMakerNext extends TileEntity implements ITeaMaker` (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileMakerNext.java:1`)
- **登録名**: `TileMakerNext` (`GameRegistry.registerTileEntity(TileMakerNext.class, "TileMakerNext")` at `CommonProxy.java:70-117`)
- **対応Block**: `TileMakerNext` の `createNewTileEntity` で生成
- **NBTキー**: `Remaining`, `Direction`, `Input` 等 (`writeToNBT`/`readFromNBT`)

## 継承・インターフェース
- 継承: `TileEntity implements ITeaMaker`
...
## レンダー
- Renderer: `TileEntityMakerNextRenderer` (ISBRH/TESR) (`ClientProxy.registerRenderers()`)
...
> 自動生成: `src/main/java/mods/defeatedcrow/common/tile/appliance/TileMakerNext.java:1` / `CommonProxy.java:70`
> 最終更新: 2026-08-24