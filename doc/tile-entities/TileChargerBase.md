...
> Source: `src/main/java/mods/defeatedcrow/common/tile/energy/TileChargerBase.java:1`
> Registry: `TileChargerBase` (`CommonProxy.java:70` `GameRegistry.registerTileEntity(TileChargerBase.class, "TileChargerBase")`)
> Block: `TileChargerBase` (`DCsAppleMilk.tilechargerbase`)
> GUI: あり (ID `6`)

## 概要
蓄電基底。ContainerBatBox/GuiBatBox ID6。 `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity` 登録。

## 登録情報
- **クラス**: `TileChargerBase extends TileEntity implements IChargeableMachine` (`src/main/java/mods/defeatedcrow/common/tile/energy/TileChargerBase.java:1`)
- **登録名**: `TileChargerBase` (`GameRegistry.registerTileEntity(TileChargerBase.class, "TileChargerBase")` at `CommonProxy.java:70-117`)
- **対応Block**: `TileChargerBase` の `createNewTileEntity` で生成
- **NBTキー**: `Remaining`, `Direction`, `Input` 等 (`writeToNBT`/`readFromNBT`)

## 継承・インターフェース
- 継承: `TileEntity implements IChargeableMachine`
...
## Container / GUI 連携
- **Container**: `ContainerChargerBase`
- **Gui**: `GuiChargerBase`
- **ID**: `6` (`CommonProxy.getServerGuiElement` / `getClientGuiElement`)
- **Open**: `player.openGui(mod, ID, world, x,y,z)` -> 1.14+ `NetworkHooks.openGui`

## レンダー
- Renderer: `TileEntityChargerBaseRenderer` (ISBRH/TESR) (`ClientProxy.registerRenderers()`)
...
> 自動生成: `src/main/java/mods/defeatedcrow/common/tile/energy/TileChargerBase.java:1` / `CommonProxy.java:70`
> 最終更新: 2026-08-24