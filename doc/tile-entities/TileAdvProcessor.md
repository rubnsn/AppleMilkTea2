...
> Source: `src/main/java/mods/defeatedcrow/common/tile/appliance/TileAdvProcessor.java:1`
> Registry: `TileAdvProcessor` (`CommonProxy.java:70` `GameRegistry.registerTileEntity(TileAdvProcessor.class, "TileAdvProcessor")`)
> Block: `TileAdvProcessor` (`DCsAppleMilk.tileadvprocessor`)
> GUI: あり (ID `5`)

## 概要
高性能プロセッサー。JawCrusher。ID5。 `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity` 登録。

## 登録情報
- **クラス**: `TileAdvProcessor extends MachineBase` (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileAdvProcessor.java:1`)
- **登録名**: `TileAdvProcessor` (`GameRegistry.registerTileEntity(TileAdvProcessor.class, "TileAdvProcessor")` at `CommonProxy.java:70-117`)
- **対応Block**: `TileAdvProcessor` の `createNewTileEntity` で生成
...
## Container / GUI 連携
- **Container**: `ContainerAdvProcessor`
- **Gui**: `GuiAdvProcessor`
- **ID**: `5` (`CommonProxy.getServerGuiElement` / `getClientGuiElement`)
- **Open**: `player.openGui(mod, ID, world, x,y,z)` -> 1.14+ `NetworkHooks.openGui`

## レンダー
- Renderer: `TileEntityAdvProcessorRenderer` (ISBRH/TESR) (`ClientProxy.registerRenderers()`)
...
> 自動生成: `src/main/java/mods/defeatedcrow/common/tile/appliance/TileAdvProcessor.java:1` / `CommonProxy.java:70`
> 最終更新: 2026-08-24