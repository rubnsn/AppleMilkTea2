...
> Category: `base / 共通基底`
> Source: `src/main/java/mods/defeatedcrow/common/tile/TileHasRemaining.java:1`
> Registry: `TileHasRemaining` (`CommonProxy.java:70` `GameRegistry.registerTileEntity(TileHasRemaining.class, "TileHasRemaining")`)
> Block: `-` (`DCsAppleMilk.-`)
> GUI: なし (ID `-`)

## 概要
残量1保持基底。remainバイト管理。 `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity` 登録。

## 登録情報
- **クラス**: `TileHasRemaining extends TileHasDirection` (`src/main/java/mods/defeatedcrow/common/tile/TileHasRemaining.java:1`)
- **登録名**: `TileHasRemaining` (`GameRegistry.registerTileEntity(TileHasRemaining.class, "TileHasRemaining")` at `CommonProxy.java:70-117`)
...
## レンダー
- Renderer: `TileEntityHasRemainingRenderer` (ISBRH/TESR) (`ClientProxy.registerRenderers()`)
...
> 自動生成: `src/main/java/mods/defeatedcrow/common/tile/TileHasRemaining.java:1` / `CommonProxy.java:70`
> 最終更新: 2026-08-24