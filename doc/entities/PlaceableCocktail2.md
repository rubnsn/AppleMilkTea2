...
> Category: `placeable`
> Source: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCocktail2.java:1`
> Registry: `defeatedcrow.PlaceableCocktail2` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdCocktail2` (default `163`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderCocktail2Entity` (`src/main/java/mods/defeatedcrow/client/entity/RenderCocktail2Entity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
設置カクテル2。cocktail2のEntity形態。 `DCsConfig.entityIdCocktail2` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `PlaceableCocktail2 extends PlaceableFoods` (`src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCocktail2.java:1`)
- **登録名**: `PlaceableCocktail2` (`EntityRegistry.registerModEntity(PlaceableCocktail2.class, "PlaceableCocktail2", DCsConfig.entityIdCocktail2, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdCocktail2 !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new PlaceableCocktail2(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderCocktail2Entity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(PlaceableCocktail2.class, new RenderCocktail2Entity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<PlaceableCocktail2>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("PlaceableCocktail2")`
- [ ] `DCsConfig.entityIdCocktail2` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdCocktail2`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCocktail2.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
