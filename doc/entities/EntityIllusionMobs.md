...
> Category: `dummy`
> Source: `src/main/java/mods/defeatedcrow/common/entity/dummy/EntityIllusionMobs.java:1`
> Registry: `defeatedcrow.illusionCreeper` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdIllusion` (default `165`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderIllusionCreeper` (`src/main/java/mods/defeatedcrow/client/entity/RenderIllusionCreeper.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
幻覚クリーパー。PotionHallucinationの視覚効果。IMobとしてスポーン取消連携。 `DCsConfig.entityIdIllusion` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `EntityIllusionMobs extends EntityLiving` (`src/main/java/mods/defeatedcrow/common/entity/dummy/EntityIllusionMobs.java:1`)
- **登録名**: `illusionCreeper` (`EntityRegistry.registerModEntity(EntityIllusionMobs.class, "illusionCreeper", DCsConfig.entityIdIllusion, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdIllusion !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
- **関連**: Villager系は Village生成連携
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new EntityIllusionMobs(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderIllusionCreeper` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(EntityIllusionMobs.class, new RenderIllusionCreeper())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<EntityIllusionMobs>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("illusionCreeper")`
- [ ] `DCsConfig.entityIdIllusion` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdIllusion`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/dummy/EntityIllusionMobs.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
