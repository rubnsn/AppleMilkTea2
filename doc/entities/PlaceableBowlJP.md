...
> Category: `placeable`
> Source: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableBowlJP.java:1`
> Registry: `defeatedcrow.PlaceableBowlJP` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdBowlJP` (default `154`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderBowlJPEntity` (`src/main/java/mods/defeatedcrow/client/entity/RenderBowlJPEntity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
設置和風碗。bowlJPのEntity形態。setCupTextureでテクスチャ切替。 `DCsConfig.entityIdBowlJP` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `PlaceableBowlJP extends PlaceableFoods` (`src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableBowlJP.java:1`)
- **登録名**: `PlaceableBowlJP` (`EntityRegistry.registerModEntity(PlaceableBowlJP.class, "PlaceableBowlJP", DCsConfig.entityIdBowlJP, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdBowlJP !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new PlaceableBowlJP(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderBowlJPEntity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(PlaceableBowlJP.class, new RenderBowlJPEntity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<PlaceableBowlJP>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("PlaceableBowlJP")`
- [ ] `DCsConfig.entityIdBowlJP` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdBowlJP`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableBowlJP.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
