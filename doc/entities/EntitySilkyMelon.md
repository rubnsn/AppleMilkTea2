...
> Category: `projectile`
> Source: `src/main/java/mods/defeatedcrow/common/entity/EntitySilkyMelon.java:1`
> Registry: `defeatedcrow.compressedSilkyMelon` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdSilkMelon` (default `161`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderSilkyMelon` (`src/main/java/mods/defeatedcrow/client/entity/RenderSilkyMelon.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
シルキーメロン。投擲爆発の強化版。fearMelon/completeFearMelonで全破壊モード。 `DCsConfig.entityIdSilkMelon` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `EntitySilkyMelon extends EntityMelonBomb` (`src/main/java/mods/defeatedcrow/common/entity/EntitySilkyMelon.java:1`)
- **登録名**: `compressedSilkyMelon` (`EntityRegistry.registerModEntity(EntitySilkyMelon.class, "compressedSilkyMelon", DCsConfig.entityIdSilkMelon, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdSilkMelon !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
- **関連**: 投擲系は Block/Item から spawn
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new EntitySilkyMelon(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderSilkyMelon` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(EntitySilkyMelon.class, new RenderSilkyMelon())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<EntitySilkyMelon>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("compressedSilkyMelon")`
- [ ] `DCsConfig.entityIdSilkMelon` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdSilkMelon`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/EntitySilkyMelon.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
