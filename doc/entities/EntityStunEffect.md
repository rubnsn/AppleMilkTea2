...
> Category: `dummy`
> Source: `src/main/java/mods/defeatedcrow/common/entity/dummy/EntityStunEffect.java:1`
> Registry: `defeatedcrow.stunEntity` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdStun` (default `164`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderStunEntity` (`src/main/java/mods/defeatedcrow/client/entity/RenderStunEntity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
スタン演出用ダミー。EntityAnchorMissileの命中演出。 `DCsConfig.entityIdStun` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `EntityStunEffect extends Entity` (`src/main/java/mods/defeatedcrow/common/entity/dummy/EntityStunEffect.java:1`)
- **登録名**: `stunEntity` (`EntityRegistry.registerModEntity(EntityStunEffect.class, "stunEntity", DCsConfig.entityIdStun, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdStun !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new EntityStunEffect(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderStunEntity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(EntityStunEffect.class, new RenderStunEntity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<EntityStunEffect>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("stunEntity")`
- [ ] `DCsConfig.entityIdStun` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdStun`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/dummy/EntityStunEffect.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
