...
> Category: `placeable`
> Source: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableAlcoholCup.java:1`
> Registry: `defeatedcrow.PlaceableAlcoholCup` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdAlcohol` (default `157`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderAlcoholCupEntity` (`src/main/java/mods/defeatedcrow/client/entity/RenderAlcoholCupEntity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
設置酒カップ。alcoholCupのEntity形態。 `DCsConfig.entityIdAlcohol` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `PlaceableAlcoholCup extends PlaceableFoods` (`src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableAlcoholCup.java:1`)
- **登録名**: `PlaceableAlcoholCup` (`EntityRegistry.registerModEntity(PlaceableAlcoholCup.class, "PlaceableAlcoholCup", DCsConfig.entityIdAlcohol, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdAlcohol !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new PlaceableAlcoholCup(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderAlcoholCupEntity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(PlaceableAlcoholCup.class, new RenderAlcoholCupEntity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<PlaceableAlcoholCup>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("PlaceableAlcoholCup")`
- [ ] `DCsConfig.entityIdAlcohol` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdAlcohol`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableAlcoholCup.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
