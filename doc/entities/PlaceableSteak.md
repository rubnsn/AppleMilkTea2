...
> Category: `placeable`
> Source: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableSteak.java:1`
> Registry: `defeatedcrow.PlaceableSteak` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdSteak` (default `155`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderSteakEntity` (`src/main/java/mods/defeatedcrow/client/entity/RenderSteakEntity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
設置ステーキ。foodPlateのEntity形態。 `DCsConfig.entityIdSteak` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `PlaceableSteak extends PlaceableFoods` (`src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableSteak.java:1`)
- **登録名**: `PlaceableSteak` (`EntityRegistry.registerModEntity(PlaceableSteak.class, "PlaceableSteak", DCsConfig.entityIdSteak, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdSteak !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new PlaceableSteak(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderSteakEntity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(PlaceableSteak.class, new RenderSteakEntity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<PlaceableSteak>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("PlaceableSteak")`
- [ ] `DCsConfig.entityIdSteak` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdSteak`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableSteak.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
