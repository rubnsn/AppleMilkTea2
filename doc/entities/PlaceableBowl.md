...
> Category: `placeable`
> Source: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableBowl.java:1`
> Registry: `defeatedcrow.PlaceableBowl` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdBowl` (default `153`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderBowlEntity` (`src/main/java/mods/defeatedcrow/client/entity/RenderBowlEntity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
設置スープ碗。bowlBlockのEntity形態。allowEdibleEntitiesでBlock/Entity切替。 `DCsConfig.entityIdBowl` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `PlaceableBowl extends PlaceableFoods` (`src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableBowl.java:1`)
- **登録名**: `PlaceableBowl` (`EntityRegistry.registerModEntity(PlaceableBowl.class, "PlaceableBowl", DCsConfig.entityIdBowl, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdBowl !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new PlaceableBowl(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderBowlEntity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(PlaceableBowl.class, new RenderBowlEntity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<PlaceableBowl>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("PlaceableBowl")`
- [ ] `DCsConfig.entityIdBowl` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdBowl`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableBowl.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
