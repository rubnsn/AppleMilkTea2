...
> Category: `placeable`
> Source: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCocktail.java:1`
> Registry: `defeatedcrow.PlaceableCocktail` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdCocktail` (default `156`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderCocktailEntity` (`src/main/java/mods/defeatedcrow/client/entity/RenderCocktailEntity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
設置カクテル1。cocktailのEntity形態。 `DCsConfig.entityIdCocktail` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `PlaceableCocktail extends PlaceableFoods` (`src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCocktail.java:1`)
- **登録名**: `PlaceableCocktail` (`EntityRegistry.registerModEntity(PlaceableCocktail.class, "PlaceableCocktail", DCsConfig.entityIdCocktail, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdCocktail !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new PlaceableCocktail(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderCocktailEntity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(PlaceableCocktail.class, new RenderCocktailEntity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<PlaceableCocktail>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("PlaceableCocktail")`
- [ ] `DCsConfig.entityIdCocktail` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdCocktail`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCocktail.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
