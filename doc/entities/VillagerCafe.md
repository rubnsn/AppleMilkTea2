...
> Category: `villager`
> Source: `src/main/java/mods/defeatedcrow/common/entity/VillagerCafe.java:1`
> Registry: `defeatedcrow.VillagerCafe` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.villagerRecipeID` (default `15`, 0=auto) (`DCsConfig.java:21`)
> Render: `-` (`src/main/java/mods/defeatedcrow/client/entity/-.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
カフェ村人。VillagerRegistryで登録。茶・食材取引。 `DCsConfig.villagerRecipeID` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `VillagerCafe implements IVillageTradeHandler` (`src/main/java/mods/defeatedcrow/common/entity/VillagerCafe.java:1`)
- **登録名**: `VillagerCafe` (`EntityRegistry.registerModEntity(VillagerCafe.class, "VillagerCafe", DCsConfig.villagerRecipeID, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.villagerRecipeID !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new VillagerCafe(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `-` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(VillagerCafe.class, new -())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<VillagerCafe>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("VillagerCafe")`
- [ ] `DCsConfig.villagerRecipeID` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.villagerRecipeID`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/VillagerCafe.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
