...
> Category: `projectile`
> Source: `src/main/java/mods/defeatedcrow/common/entity/EntityYuzuBullet.java:1`
> Registry: `defeatedcrow.yuzuBullet` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdBullet` (default `167`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderYuzuBullet` (`src/main/java/mods/defeatedcrow/client/entity/RenderYuzuBullet.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
柚子弾。yuzuGatlingから連射。damage=5.0。PvPProhibitionModeで対人無効。 `DCsConfig.entityIdBullet` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `EntityYuzuBullet extends Entity implements IProjectile` (`src/main/java/mods/defeatedcrow/common/entity/EntityYuzuBullet.java:1`)
- **登録名**: `yuzuBullet` (`EntityRegistry.registerModEntity(EntityYuzuBullet.class, "yuzuBullet", DCsConfig.entityIdBullet, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdBullet !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new EntityYuzuBullet(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderYuzuBullet` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(EntityYuzuBullet.class, new RenderYuzuBullet())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<EntityYuzuBullet>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("yuzuBullet")`
- [ ] `DCsConfig.entityIdBullet` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdBullet`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/EntityYuzuBullet.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
