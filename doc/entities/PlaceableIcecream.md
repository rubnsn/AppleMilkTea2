...
> Category: `placeable`
> Source: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableIcecream.java:1`
> Registry: `defeatedcrow.PlaceableIceCream` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdIce` (default `150`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderIceCreamEntity` (`src/main/java/mods/defeatedcrow/client/entity/RenderIceCreamEntity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
設置アイス。blockIcecreamのEntity形態。rightClickで食べる。 `DCsConfig.entityIdIce` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `PlaceableIcecream extends PlaceableFoods` (`src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableIcecream.java:1`)
- **登録名**: `PlaceableIceCream` (`EntityRegistry.registerModEntity(PlaceableIcecream.class, "PlaceableIceCream", DCsConfig.entityIdIce, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdIce !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new PlaceableIcecream(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderIceCreamEntity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(PlaceableIcecream.class, new RenderIceCreamEntity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<PlaceableIcecream>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("PlaceableIceCream")`
- [ ] `DCsConfig.entityIdIce` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdIce`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableIcecream.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
