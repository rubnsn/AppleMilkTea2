...
> Category: `placeable`
> Source: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCup2.java:1`
> Registry: `defeatedcrow.PlaceableCup2` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdCup2` (default `152`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderCup2Entity` (`src/main/java/mods/defeatedcrow/client/entity/RenderCup2Entity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
設置ティーカップ2。teaCup2のEntity形態。 `DCsConfig.entityIdCup2` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `PlaceableCup2 extends PlaceableFoods` (`src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCup2.java:1`)
- **登録名**: `PlaceableCup2` (`EntityRegistry.registerModEntity(PlaceableCup2.class, "PlaceableCup2", DCsConfig.entityIdCup2, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdCup2 !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new PlaceableCup2(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderCup2Entity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(PlaceableCup2.class, new RenderCup2Entity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<PlaceableCup2>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("PlaceableCup2")`
- [ ] `DCsConfig.entityIdCup2` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdCup2`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCup2.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
