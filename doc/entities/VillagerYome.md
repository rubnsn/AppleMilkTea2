...
> Category: `villager`
> Source: `src/main/java/mods/defeatedcrow/common/entity/VillagerYome.java:1`
> Registry: `defeatedcrow.VillagerYome` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.villagerRecipe2ID` (default `16`, 0=auto) (`DCsConfig.java:21`)
...
## 概要
ヨメ村人。追加取引担当。 `DCsConfig.villagerRecipe2ID` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `VillagerYome implements IVillageTradeHandler` (`src/main/java/mods/defeatedcrow/common/entity/VillagerYome.java:1`)
- **登録名**: `VillagerYome` (`EntityRegistry.registerModEntity(VillagerYome.class, "VillagerYome", DCsConfig.villagerRecipe2ID, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.villagerRecipe2ID !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new VillagerYome(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `-` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(VillagerYome.class, new -())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<VillagerYome>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("VillagerYome")`
- [ ] `DCsConfig.villagerRecipe2ID` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.villagerRecipe2ID`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/VillagerYome.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
