...
> Category: `projectile`
> Source: `src/main/java/mods/defeatedcrow/common/entity/EntityAnchorMissile.java:1`
> Registry: `defeatedcrow.anchorMissile` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdMissile` (default `166`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderAnchorMissile` (`src/main/java/mods/defeatedcrow/client/entity/RenderAnchorMissile.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
アンカーミサイル。fossilCannonから発射。disableMissileExplosionで爆発無効可。 `DCsConfig.entityIdMissile` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `EntityAnchorMissile extends Entity` (`src/main/java/mods/defeatedcrow/common/entity/EntityAnchorMissile.java:1`)
- **登録名**: `anchorMissile` (`EntityRegistry.registerModEntity(EntityAnchorMissile.class, "anchorMissile", DCsConfig.entityIdMissile, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdMissile !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new EntityAnchorMissile(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderAnchorMissile` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(EntityAnchorMissile.class, new RenderAnchorMissile())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<EntityAnchorMissile>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("anchorMissile")`
- [ ] `DCsConfig.entityIdMissile` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdMissile`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/EntityAnchorMissile.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
