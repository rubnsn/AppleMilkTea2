...
> Category: `placeable`
> Source: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableBaseSoup.java:1`
> Registry: `defeatedcrow.PlaceableBaseSoup` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdBaseSoup` (default `169`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderBaseSoupEntity` (`src/main/java/mods/defeatedcrow/client/entity/RenderBaseSoupEntity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
設置ベーススープ。ItemBaseSoupBowlのEntity形態。 `DCsConfig.entityIdBaseSoup` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `PlaceableBaseSoup extends PlaceableFoods` (`src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableBaseSoup.java:1`)
- **登録名**: `PlaceableBaseSoup` (`EntityRegistry.registerModEntity(PlaceableBaseSoup.class, "PlaceableBaseSoup", DCsConfig.entityIdBaseSoup, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdBaseSoup !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new PlaceableBaseSoup(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderBaseSoupEntity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(PlaceableBaseSoup.class, new RenderBaseSoupEntity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<PlaceableBaseSoup>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("PlaceableBaseSoup")`
- [ ] `DCsConfig.entityIdBaseSoup` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdBaseSoup`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableBaseSoup.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
