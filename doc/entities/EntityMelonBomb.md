...
> Category: `projectile`
> Source: `src/main/java/mods/defeatedcrow/common/entity/EntityMelonBomb.java:1`
> Registry: `defeatedcrow.compressedMelon` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdMelon` (default `160`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderMelonBomb` (`src/main/java/mods/defeatedcrow/client/entity/RenderMelonBomb.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
スイカ爆弾。投擲→着弾で爆発。canExplodeMelon/melonBreakBlockで挙動可変。 `DCsConfig.entityIdMelon` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `EntityMelonBomb extends Entity` (`src/main/java/mods/defeatedcrow/common/entity/EntityMelonBomb.java:1`)
- **登録名**: `compressedMelon` (`EntityRegistry.registerModEntity(EntityMelonBomb.class, "compressedMelon", DCsConfig.entityIdMelon, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdMelon !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new EntityMelonBomb(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderMelonBomb` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(EntityMelonBomb.class, new RenderMelonBomb())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<EntityMelonBomb>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("compressedMelon")`
- [ ] `DCsConfig.entityIdMelon` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdMelon`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/EntityMelonBomb.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
