...
> Category: `placeable`
> Source: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableTart.java:1`
> Registry: `defeatedcrow.PlaceableTart` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdTart` (default `159`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderTartEntity` (`src/main/java/mods/defeatedcrow/client/entity/RenderTartEntity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
設置タルト。ItemAppleTartのEntity形態。 `DCsConfig.entityIdTart` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `PlaceableTart extends PlaceableFoods` (`src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableTart.java:1`)
- **登録名**: `PlaceableTart` (`EntityRegistry.registerModEntity(PlaceableTart.class, "PlaceableTart", DCsConfig.entityIdTart, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdTart !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new PlaceableTart(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderTartEntity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(PlaceableTart.class, new RenderTartEntity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<PlaceableTart>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("PlaceableTart")`
- [ ] `DCsConfig.entityIdTart` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdTart`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableTart.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
