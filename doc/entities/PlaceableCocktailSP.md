...
> Category: `placeable`
> Source: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCocktailSP.java:1`
> Registry: `defeatedcrow.PlaceableCocktailSP` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdCocktailSP` (default `168`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderCocktailSPEntity` (`src/main/java/mods/defeatedcrow/client/entity/RenderCocktailSPEntity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
設置カクテルSP。cocktailSPのEntity形態。 `DCsConfig.entityIdCocktailSP` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `PlaceableCocktailSP extends PlaceableFoods` (`src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCocktailSP.java:1`)
- **登録名**: `PlaceableCocktailSP` (`EntityRegistry.registerModEntity(PlaceableCocktailSP.class, "PlaceableCocktailSP", DCsConfig.entityIdCocktailSP, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdCocktailSP !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new PlaceableCocktailSP(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderCocktailSPEntity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(PlaceableCocktailSP.class, new RenderCocktailSPEntity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<PlaceableCocktailSP>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("PlaceableCocktailSP")`
- [ ] `DCsConfig.entityIdCocktailSP` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdCocktailSP`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/edible/PlaceableCocktailSP.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
