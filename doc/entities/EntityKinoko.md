...
> Category: `projectile`
> Source: `src/main/java/mods/defeatedcrow/common/entity/EntityKinoko.java:1`
> Registry: `defeatedcrow.mushroomBox` (`DCsAppleMilk.java:542` `EntityRegistry.registerModEntity`)
> Config: `DCsConfig.entityIdKinoko` (default `162`, 0=auto) (`DCsConfig.java:21`)
> Render: `RenderKinokoEntity` (`src/main/java/mods/defeatedcrow/client/entity/RenderKinokoEntity.java:1` if exists)
> Tracking: `range=250, frequency=5, velocityUpdates=true` (全Entity共通)

## 概要
キノコ箱投擲体。BlockMushBoxから投射。 `DCsConfig.entityIdKinoko` でID可変、0時は `findGlobalUniqueEntityId()` で自動採番。

## 登録情報
- **クラス**: `EntityKinoko extends Entity` (`src/main/java/mods/defeatedcrow/common/entity/EntityKinoko.java:1`)
- **登録名**: `mushroomBox` (`EntityRegistry.registerModEntity(EntityKinoko.class, "mushroomBox", DCsConfig.entityIdKinoko, mod, 250, 5, true)` at `DCsAppleMilk.java:542-702`)
- **ID解決**: `DCsConfig.entityIdKinoko !=0 ? config値 : EntityRegistry.findGlobalUniqueEntityId()` のパターン全Entity共通
...
- **Placeable**: `ItemBlock` の `onItemUse` で `world.spawnEntityInWorld(new EntityKinoko(world, stack, x,y,z))` 生成

## レンダー
- Renderer: `RenderKinokoEntity` (`ClientProxy.registerRenderers()` で `RenderingRegistry.registerEntityRenderingHandler(EntityKinoko.class, new RenderKinokoEntity())`)
...
- [ ] `EntityRegistry.registerModEntity` → `DeferredRegister<EntityType<?>>` + `EntityType.Builder.<EntityKinoko>of(...).sized(w,h).clientTrackingRange(250).updateInterval(5).build("mushroomBox")`
- [ ] `DCsConfig.entityIdKinoko` 整数ID → 削除（ResourceLocationベース）
...
- [Config](../config.md) - `DCsConfig.entityIdKinoko`

> 自動生成: `src/main/java/mods/defeatedcrow/common/entity/EntityKinoko.java:1` / `DCsAppleMilk.java:542`
> 最終更新: 2026-08-24
