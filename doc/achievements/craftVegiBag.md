# craftVegiBag (dc.craftVegiBag)

> Source: `src/main/java/mods/defeatedcrow/common/AchievementRegister.java:1`
> Registry: `new Achievement("defeatedcrow.vegibag", "dc.craftVegiBag", 1, 5, new ItemStack(DCsAppleMilk.vegiBag), parent).registerStat()` (`AchievementRegister.java:58`)
> Parent: `craftLogBox`
> Display: `ItemStack(DCsAppleMilk.vegiBag)` at (1,5)
...
- **ID**: `"defeatedcrow.vegibag"` (stat) / `"dc.craftVegiBag"` (name)
- **位置**: `(1,5)` グリッド
- **アイコン**: `new ItemStack(DCsAppleMilk.vegiBag)`
...
| `player.triggerAchievement(AchievementRegister.craftVegiBag)` | `AdvancementManager` + `ServerPlayer` の `AdvancementProgress.grantCriterion` / `CriterionTrigger` | 同左 |
| `AchievementList.openInventory` 等 | `Advancement` の `parent` に `minecraft:story/root` 等を指定 | 同左 |

移行例 (1.16+ JSON):
```json
{
  "display": { "icon": { "item": "defeatedcrow:teamaker_next" }, "title": { "translate": "advancement.defeatedcrow.craftVegiBag.title" }, "frame_type": "task" },
