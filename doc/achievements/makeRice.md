# makeRice (dc.makeRice)

> Source: `src/main/java/mods/defeatedcrow/common/AchievementRegister.java:1`
> Registry: `new Achievement("defeatedcrow.rice", "dc.makeRice", -6, 1, new ItemStack(DCsAppleMilk.emptyPanGaiden), parent).registerStat()` (`AchievementRegister.java:58`)
> Parent: `craftGrater`
> Display: `ItemStack(DCsAppleMilk.emptyPanGaiden)` at (-6,1)
...
- **ID**: `"defeatedcrow.rice"` (stat) / `"dc.makeRice"` (name)
- **位置**: `(-6,1)` グリッド
...
| `player.triggerAchievement(AchievementRegister.makeRice)` | `AdvancementManager` + `ServerPlayer` の `AdvancementProgress.grantCriterion` / `CriterionTrigger` | 同左 |
| `AchievementList.openInventory` 等 | `Advancement` の `parent` に `minecraft:story/root` 等を指定 | 同左 |

移行例 (1.16+ JSON):
```json
{
  "display": { "icon": { "item": "defeatedcrow:teamaker_next" }, "title": { "translate": "advancement.defeatedcrow.makeRice.title" }, "frame_type": "task" },
