# craftChalcedony (dc.craftChalcedony)

> Source: `src/main/java/mods/defeatedcrow/common/AchievementRegister.java:1`
> Registry: `new Achievement("defeatedcrow.chalcedony", "dc.craftChalcedony", 2, 2, new ItemStack(DCsAppleMilk.chalcedony), parent).registerStat()` (`AchievementRegister.java:58`)
> Parent: `AchievementList.buildFurnace`
> Display: `ItemStack(DCsAppleMilk.chalcedony)` at (2,2)
...
- **ID**: `"defeatedcrow.chalcedony"` (stat) / `"dc.craftChalcedony"` (name)
- **位置**: `(2,2)` グリッド
- **アイコン**: `new ItemStack(DCsAppleMilk.chalcedony)`
...
| `player.triggerAchievement(AchievementRegister.craftChalcedony)` | `AdvancementManager` + `ServerPlayer` の `AdvancementProgress.grantCriterion` / `CriterionTrigger` | 同左 |
| `AchievementList.openInventory` 等 | `Advancement` の `parent` に `minecraft:story/root` 等を指定 | 同左 |

移行例 (1.16+ JSON):
```json
{
  "display": { "icon": { "item": "defeatedcrow:teamaker_next" }, "title": { "translate": "advancement.defeatedcrow.craftChalcedony.title" }, "frame_type": "task" },
