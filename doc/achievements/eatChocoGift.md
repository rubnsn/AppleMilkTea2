# eatChocoGift (dc.eatChocoGift)

> Source: `src/main/java/mods/defeatedcrow/common/AchievementRegister.java:1`
> Registry: `new Achievement("defeatedcrow.chocolateGift", "dc.eatChocoGift", -9, -1, new ItemStack(DCsAppleMilk.chocoBlock), parent).registerStat()` (`AchievementRegister.java:58`)
> Parent: `makeRice`
> Display: `ItemStack(DCsAppleMilk.chocoBlock)` at (-9,-1)
...
- **ID**: `"defeatedcrow.chocolateGift"` (stat) / `"dc.eatChocoGift"` (name)
- **位置**: `(-9,-1)` グリッド
- **アイコン**: `new ItemStack(DCsAppleMilk.chocoBlock)`
...
| `player.triggerAchievement(AchievementRegister.eatChocoGift)` | `AdvancementManager` + `ServerPlayer` の `AdvancementProgress.grantCriterion` / `CriterionTrigger` | 同左 |
| `AchievementList.openInventory` 等 | `Advancement` の `parent` に `minecraft:story/root` 等を指定 | 同左 |

移行例 (1.16+ JSON):
```json
{
  "display": { "icon": { "item": "defeatedcrow:teamaker_next" }, "title": { "translate": "advancement.defeatedcrow.eatChocoGift.title" }, "frame_type": "challenge" },
