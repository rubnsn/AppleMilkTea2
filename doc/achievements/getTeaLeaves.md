# getTeaLeaves (dc.getTeaLeaves)

> Source: `src/main/java/mods/defeatedcrow/common/AchievementRegister.java:1`
> Registry: `new Achievement("defeatedcrow.getTeaLeaves", "dc.getTeaLeaves", 0, 1, new ItemStack(DCsAppleMilk.leafTea:0), parent).registerStat()` (`AchievementRegister.java:58`)
> Parent: `AchievementList.openInventory`
> Display: `ItemStack(DCsAppleMilk.leafTea:0)` at (0,1)
...
- **ID**: `"defeatedcrow.getTeaLeaves"` (stat) / `"dc.getTeaLeaves"` (name)
- **位置**: `(0,1)` グリッド
- **アイコン**: `new ItemStack(DCsAppleMilk.leafTea:0)`
...
| `player.triggerAchievement(AchievementRegister.getTeaLeaves)` | `AdvancementManager` + `ServerPlayer` の `AdvancementProgress.grantCriterion` / `CriterionTrigger` | 同左 |
| `AchievementList.openInventory` 等 | `Advancement` の `parent` に `minecraft:story/root` 等を指定 | 同左 |

移行例 (1.16+ JSON):
```json
{
  "display": { "icon": { "item": "defeatedcrow:teamaker_next" }, "title": { "translate": "advancement.defeatedcrow.getTeaLeaves.title" }, "frame_type": "task" },
