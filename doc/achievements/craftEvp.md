# craftEvp (dc.craftEvaporator)

> Source: `src/main/java/mods/defeatedcrow/common/AchievementRegister.java:1`
> Registry: `new Achievement("defeatedcrow.evaporator", "dc.craftEvaporator", 6, -1, new ItemStack(DCsAppleMilk.evaporator), parent).registerStat()` (`AchievementRegister.java:58`)
> Parent: `getAlcohol`
> Display: `ItemStack(DCsAppleMilk.evaporator)` at (6,-1)
...
- **ID**: `"defeatedcrow.evaporator"` (stat) / `"dc.craftEvaporator"` (name)
- **位置**: `(6,-1)` グリッド
- **アイコン**: `new ItemStack(DCsAppleMilk.evaporator)`
...
| `player.triggerAchievement(AchievementRegister.craftEvp)` | `AdvancementManager` + `ServerPlayer` の `AdvancementProgress.grantCriterion` / `CriterionTrigger` | 同左 |
| `AchievementList.openInventory` 等 | `Advancement` の `parent` に `minecraft:story/root` 等を指定 | 同左 |

移行例 (1.16+ JSON):
```json
{
  "display": { "icon": { "item": "defeatedcrow:teamaker_next" }, "title": { "translate": "advancement.defeatedcrow.craftEvp.title" }, "frame_type": "task" },
