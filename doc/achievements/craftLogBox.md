# craftLogBox (dc.craftLogbox)

> Source: `src/main/java/mods/defeatedcrow/common/AchievementRegister.java:1`
> Registry: `new Achievement("defeatedcrow.logbox", "dc.craftLogbox", 0, 3, new ItemStack(DCsAppleMilk.woodBox), parent).registerStat()` (`AchievementRegister.java:58`)
> Parent: `AchievementList.buildWorkBench`
> Display: `ItemStack(DCsAppleMilk.woodBox)` at (0,3)
...
- **ID**: `"defeatedcrow.logbox"` (stat) / `"dc.craftLogbox"` (name)
- **位置**: `(0,3)` グリッド
- **アイコン**: `new ItemStack(DCsAppleMilk.woodBox)`
...
| `player.triggerAchievement(AchievementRegister.craftLogBox)` | `AdvancementManager` + `ServerPlayer` の `AdvancementProgress.grantCriterion` / `CriterionTrigger` | 同左 |
| `AchievementList.openInventory` 等 | `Advancement` の `parent` に `minecraft:story/root` 等を指定 | 同左 |

移行例 (1.16+ JSON):
```json
{
  "display": { "icon": { "item": "defeatedcrow:teamaker_next" }, "title": { "translate": "advancement.defeatedcrow.craftLogBox.title" }, "frame_type": "task" },
