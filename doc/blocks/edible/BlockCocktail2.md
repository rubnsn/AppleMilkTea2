...
> Category: `edible`  
> Source: `src/main/java/mods/defeatedcrow/common/block/edible/BlockCocktail2.java:1`  
> Registry: `defeatedcrow.cocktail2` (`DCsAppleMilk.cocktail2`)  
> ItemBlock: `EntityItemCocktail2` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemCocktail2.java:1`)  
> TileEntity: `TileCocktail2` (`TileCocktail2`) (`src/main/java/mods/defeatedcrow/common/tile/TileCocktail2.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:177` `public static Block cocktail2;`
- **インスタンス生成**: `MaterialRegister.java:839` `new BlockCocktail2()` / `setBlockName("defeatedcrow.cocktail2")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.cocktail2, EntityItemCocktail2.class, "defeatedcrow.cocktail2")` (`MaterialRegister.java:409`)
- **Material / Hardness / Resistance**: `Material.glass`, hardness `0.2F`, resistance `1.0F` (`BlockCocktail2.java:1`)
...
- Tile: `TileCocktail2` (`src/main/java/mods/defeatedcrow/common/tile/TileCocktail2.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileCocktail2.class, "TileCocktail2")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelCocktail2`使用。
- ItemBlock: `EntityItemCocktail2` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemCocktail2.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelCocktail2` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityCocktail2Renderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
