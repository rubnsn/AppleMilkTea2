...
> Category: `edible`  
> Source: `src/main/java/mods/defeatedcrow/common/block/edible/BlockFilledCup.java:1`  
> Registry: `defeatedcrow.filledCup` (`DCsAppleMilk.teacupBlock`)  
> ItemBlock: `EntityItemTeaCup` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemTeaCup.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:173` `public static Block teacupBlock;`
- **インスタンス生成**: `MaterialRegister.java:816` `new BlockFilledCup()` / `setBlockName("defeatedcrow.filledCup")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.teacupBlock, EntityItemTeaCup.class, "defeatedcrow.filledCup")` (`MaterialRegister.java:404`)
- **Material / Hardness / Resistance**: `Material.circuits`, hardness ``, resistance `` (`BlockFilledCup.java:1`)
...
- Tile: `TileCupHandle` (`src/main/java/mods/defeatedcrow/common/tile/TileCupHandle.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileCupHandle.class, "TileCupHandle")`
  ...
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelFilledCup`使用。
- ItemBlock: `EntityItemTeaCup` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemTeaCup.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelFilledCup` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityFilledCupRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
