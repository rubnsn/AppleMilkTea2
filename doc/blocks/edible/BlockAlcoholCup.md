...
> Category: `edible`  
> Source: `src/main/java/mods/defeatedcrow/common/block/edible/BlockAlcoholCup.java:1`  
> Registry: `defeatedcrow.alcoholCup` (`DCsAppleMilk.alcoholCup`)  
> ItemBlock: `EntityItemAlcoholCup` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemAlcoholCup.java:1`)  
> TileEntity: `TileAlcoholCup` (`TileAlcoholCup`) (`src/main/java/mods/defeatedcrow/common/tile/TileAlcoholCup.java:1`)  
> CreativeTab: `applemilkFood`

## 概要
設置して食べられる食物ブロック。`BlockContainer`+`TileHasDirection`系で向き・残量を管理。右クリックでEntity化して回収。

## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:178` `public static Block alcoholCup;`
- **インスタンス生成**: `MaterialRegister.java:833` `new BlockAlcoholCup()` / `setBlockName("defeatedcrow.alcoholCup")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.alcoholCup, EntityItemAlcoholCup.class, "defeatedcrow.alcoholCup")` (`MaterialRegister.java:407`)
- **Material / Hardness / Resistance**: `Material.glass`, hardness `0.2F`, resistance `1.0F` (`BlockAlcoholCup.java:1`)
...
- Tile: `TileAlcoholCup` (`src/main/java/mods/defeatedcrow/common/tile/TileAlcoholCup.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileAlcoholCup.class, "TileAlcoholCup")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelAlcoholCup`使用。
- ItemBlock: `EntityItemAlcoholCup` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemAlcoholCup.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelAlcoholCup` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityAlcoholCupRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
