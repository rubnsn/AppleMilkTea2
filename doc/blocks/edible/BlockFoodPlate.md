...
> Category: `edible`  
> Source: `src/main/java/mods/defeatedcrow/common/block/edible/BlockFoodPlate.java:1`  
> Registry: `defeatedcrow.foodPlate` (`DCsAppleMilk.foodPlate`)  
> ItemBlock: `EntityItemSteak` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemSteak.java:1`)  
> TileEntity: `TileSteak` (`TileSteak`) (`src/main/java/mods/defeatedcrow/common/tile/TileSteak.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:181` `public static Block foodPlate;`
- **インスタンス生成**: `MaterialRegister.java:810` `new BlockFoodPlate()` / `setBlockName("defeatedcrow.foodPlate")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.foodPlate, EntityItemSteak.class, "defeatedcrow.foodPlate")` (`MaterialRegister.java:402`)
- **Material / Hardness / Resistance**: `Material.circuits`, hardness `0.2F`, resistance `1.0F` (`BlockFoodPlate.java:1`)
...
- Tile: `TileSteak` (`src/main/java/mods/defeatedcrow/common/tile/TileSteak.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileSteak.class, "TileSteak")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelFoodPlate`使用。
- ItemBlock: `EntityItemSteak` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemSteak.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelFoodPlate` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityFoodPlateRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
