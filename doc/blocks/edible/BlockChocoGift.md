...
> Category: `edible`  
> Source: `src/main/java/mods/defeatedcrow/common/block/edible/BlockChocoGift.java:1`  
> Registry: `defeatedcrow.chocolateGift` (`DCsAppleMilk.chocoBlock`)  
> ItemBlock: `ItemChocoGift` (`src/main/java/mods/defeatedcrow/common/block/edible/ItemChocoGift.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:182` `public static Block chocoBlock;`
- **インスタンス生成**: `MaterialRegister.java:813` `new BlockChocoGift()` / `setBlockName("defeatedcrow.chocolateGift")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.chocoBlock, ItemChocoGift.class, "defeatedcrow.chocolateGift")` (`MaterialRegister.java:403`)
- **Material / Hardness / Resistance**: `Material.circuits`, hardness `0.2F`, resistance `0.0F` (`BlockChocoGift.java:1`)
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemChocoGift` (`src/main/java/mods/defeatedcrow/common/block/edible/ItemChocoGift.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelChocoGift` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
