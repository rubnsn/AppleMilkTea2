...
> Category: `edible`  
> Source: `src/main/java/mods/defeatedcrow/common/block/edible/BlockBowlJP.java:1`  
> Registry: `defeatedcrow.bowlJP` (`DCsAppleMilk.bowlJP`)  
> ItemBlock: `EntityItemBowlJP` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemBowlJP.java:1`)  
> TileEntity: `TileJPBowl` (`TileJPBowl`) (`src/main/java/mods/defeatedcrow/common/tile/TileJPBowl.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:180` `public static Block bowlJP;`
- **インスタンス生成**: `MaterialRegister.java:807` `new BlockBowlJP()` / `setBlockName("defeatedcrow.bowlJP")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.bowlJP, EntityItemBowlJP.class, "defeatedcrow.bowlJP")` (`MaterialRegister.java:401`)
- **Material / Hardness / Resistance**: `Material.circuits`, hardness `0.2F`, resistance `1.0F` (`BlockBowlJP.java:1`)
...
- Tile: `TileJPBowl` (`src/main/java/mods/defeatedcrow/common/tile/TileJPBowl.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileJPBowl.class, "TileJPBowl")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelBowlJP`使用。
- ItemBlock: `EntityItemBowlJP` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemBowlJP.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelBowlJP` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityBowlJPRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
