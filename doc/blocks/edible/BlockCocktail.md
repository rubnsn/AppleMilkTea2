...
> Category: `edible`  
> Source: `src/main/java/mods/defeatedcrow/common/block/edible/BlockCocktail.java:1`  
> Registry: `defeatedcrow.cocktail` (`DCsAppleMilk.cocktail`)  
> ItemBlock: `EntityItemCocktail` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemCocktail.java:1`)  
> TileEntity: `TileCocktail` (`TileCocktail`) (`src/main/java/mods/defeatedcrow/common/tile/TileCocktail.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:176` `public static Block cocktail;`
- **インスタンス生成**: `MaterialRegister.java:836` `new BlockCocktail()` / `setBlockName("defeatedcrow.cocktail")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.cocktail, EntityItemCocktail.class, "defeatedcrow.cocktail")` (`MaterialRegister.java:408`)
- **Material / Hardness / Resistance**: `Material.glass`, hardness `0.2F`, resistance `1.0F` (`BlockCocktail.java:1`)
...
- Tile: `TileCocktail` (`src/main/java/mods/defeatedcrow/common/tile/TileCocktail.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileCocktail.class, "TileCocktail")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelCocktail`使用。
- ItemBlock: `EntityItemCocktail` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemCocktail.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelCocktail` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityCocktailRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
