...
> Category: `edible`  
> Source: `src/main/java/mods/defeatedcrow/common/block/edible/BlockIceCream.java:1`  
> Registry: `defeatedcrow.iceCreamBlock` (`DCsAppleMilk.blockIcecream`)  
> ItemBlock: `EntityItemIceCream` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemIceCream.java:1`)  
> TileEntity: `TileIceCream` (`TileIcecream`) (`src/main/java/mods/defeatedcrow/common/tile/TileIceCream.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:175` `public static Block blockIcecream;`
- **インスタンス生成**: `MaterialRegister.java:830` `new BlockIceCream()` / `setBlockName("defeatedcrow.iceCreamBlock")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.blockIcecream, EntityItemIceCream.class, "defeatedcrow.iceCreamBlock")` (`MaterialRegister.java:406`)
- **Material / Hardness / Resistance**: `Material.glass`, hardness `0.2F`, resistance `1.0F` (`BlockIceCream.java:1`)
...
- Tile: `TileIceCream` (`src/main/java/mods/defeatedcrow/common/tile/TileIceCream.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileIceCream.class, "TileIcecream")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelIceCream`使用。
- ItemBlock: `EntityItemIceCream` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemIceCream.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelIceCream` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityIceCreamRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
