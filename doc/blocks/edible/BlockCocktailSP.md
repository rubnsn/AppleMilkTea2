...
> Category: `edible`  
> Source: `src/main/java/mods/defeatedcrow/common/block/edible/BlockCocktailSP.java:1`  
> Registry: `defeatedcrow.cocktailSP` (`DCsAppleMilk.cocktailSP`)  
> ItemBlock: `EntityItemCocktailSP` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemCocktailSP.java:1`)  
> TileEntity: `TileCocktailSP` (`TileCocktailSP`) (`src/main/java/mods/defeatedcrow/common/tile/TileCocktailSP.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:183` `public static Block cocktailSP;`
- **インスタンス生成**: `MaterialRegister.java:842` `new BlockCocktailSP()` / `setBlockName("defeatedcrow.cocktailSP")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.cocktailSP, EntityItemCocktailSP.class, "defeatedcrow.cocktailSP")` (`MaterialRegister.java:410`)
- **Material / Hardness / Resistance**: `Material.glass`, hardness `0.2F`, resistance `1.0F` (`BlockCocktailSP.java:1`)
...
- Tile: `TileCocktailSP` (`src/main/java/mods/defeatedcrow/common/tile/TileCocktailSP.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileCocktailSP.class, "TileCocktailSP")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelCocktailSP`使用。
- ItemBlock: `EntityItemCocktailSP` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemCocktailSP.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelCocktailSP` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityCocktailSPRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
