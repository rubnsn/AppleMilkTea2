...
> Category: `edible`  
> Source: `src/main/java/mods/defeatedcrow/common/block/edible/BlockFilledCup2.java:1`  
> Registry: `defeatedcrow.filledCup2` (`DCsAppleMilk.teaCup2`)  
> ItemBlock: `EntityItemTeaCup2` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemTeaCup2.java:1`)  
> TileEntity: `TileCupHandle` (`TileCupHandle`) (`src/main/java/mods/defeatedcrow/common/tile/TileCupHandle.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:174` `public static Block teaCup2;`
- **インスタンス生成**: `MaterialRegister.java:819` `new BlockFilledCup2()` / `setBlockName("defeatedcrow.filledCup2")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.teaCup2, EntityItemTeaCup2.class, "defeatedcrow.filledCup2")` (`MaterialRegister.java:405`)
- **Material / Hardness / Resistance**: `Material.circuits`, hardness ``, resistance `` (`BlockFilledCup2.java:1`)
...
- Tile: `TileCupHandle` (`src/main/java/mods/defeatedcrow/common/tile/TileCupHandle.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileCupHandle.class, "TileCupHandle")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelFilledCup2`使用。
- ItemBlock: `EntityItemTeaCup2` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemTeaCup2.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelFilledCup2` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityFilledCup2Renderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
