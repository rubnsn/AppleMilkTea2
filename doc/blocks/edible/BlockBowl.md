...
> Category: `edible`  
> Source: `src/main/java/mods/defeatedcrow/common/block/edible/BlockBowl.java:1`  
> Registry: `defeatedcrow.bowlBlock` (`DCsAppleMilk.bowlBlock`)  
> ItemBlock: `EntityItemBowl` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemBowl.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:179` `public static Block bowlBlock;`
- **インスタンス生成**: `MaterialRegister.java:804` `new BlockBowl()` / `setBlockName("defeatedcrow.bowlBlock")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.bowlBlock, EntityItemBowl.class, "defeatedcrow.bowlBlock")` (`MaterialRegister.java:400`)
- **Material / Hardness / Resistance**: `Material.circuits`, hardness `0.2F`, resistance `1.0F` (`BlockBowl.java:1`)
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `EntityItemBowl` (`src/main/java/mods/defeatedcrow/common/block/edible/EntityItemBowl.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelBowl` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
