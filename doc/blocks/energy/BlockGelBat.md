...
> Category: `energy`  
> Source: `src/main/java/mods/defeatedcrow/common/block/energy/BlockGelBat.java:1`  
> Registry: `defeatedcrow.gelBatContainer` (`DCsAppleMilk.gelBat`)  
> ItemBlock: `ItemGelBat` (`src/main/java/mods/defeatedcrow/common/block/energy/ItemGelBat.java:1`)  
> TileEntity: `TileGelBat` (`TileGelBattery`) (`src/main/java/mods/defeatedcrow/common/tile/energy/TileGelBat.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:169` `public static Block gelBat;`
- **インスタンス生成**: `MaterialRegister.java:1085` `new BlockGelBat()` / `setBlockName("defeatedcrow.gelBatContainer")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.gelBat, ItemGelBat.class, "defeatedcrow.gelBatContainer")` (`MaterialRegister.java:332`)
- **Material / Hardness / Resistance**: `Material.ground`, hardness `1.0F`, resistance `2.0F` (`BlockGelBat.java:1`)
...
- Tile: `TileGelBat` (`src/main/java/mods/defeatedcrow/common/tile/TileGelBat.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileGelBat.class, "TileGelBattery")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelGelBat`使用。
- ItemBlock: `ItemGelBat` (`src/main/java/mods/defeatedcrow/common/block/energy/ItemGelBat.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelGelBat` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityGelBatRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
