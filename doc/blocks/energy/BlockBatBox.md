...
> Category: `energy`  
> Source: `src/main/java/mods/defeatedcrow/common/block/energy/BlockBatBox.java:1`  
> Registry: `defeatedcrow.batBox` (`DCsAppleMilk.batBox`)  
> ItemBlock: `ItemBatBox` (`src/main/java/mods/defeatedcrow/common/block/energy/ItemBatBox.java:1`)  
> TileEntity: `TileChargerDevice` (`TileChargerDevice`) (`src/main/java/mods/defeatedcrow/common/tile/energy/TileChargerDevice.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:165` `public static Block batBox;`
- **インスタンス生成**: `MaterialRegister.java:1094` `new BlockBatBox()` / `setBlockName("defeatedcrow.batBox")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.batBox, ItemBatBox.class, "defeatedcrow.batBox")` (`MaterialRegister.java:333`)
- **Material / Hardness / Resistance**: `Material.ground`, hardness `2.0F`, resistance `2.0F` (`BlockBatBox.java:1`)
...
- Tile: `TileChargerDevice` (`src/main/java/mods/defeatedcrow/common/tile/TileChargerDevice.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileChargerDevice.class, "TileChargerDevice")`
  - GUI: `充電デバイス (TileChargerBase継承、GUIなし)` (`CommonProxy.getServerGuiElement/getClientGuiElement:1`)
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelBatBox`使用。
- ItemBlock: `ItemBatBox` (`src/main/java/mods/defeatedcrow/common/block/energy/ItemBatBox.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelBatBox` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityBatBoxRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
