...
> Category: `energy`  
> Source: `src/main/java/mods/defeatedcrow/common/block/energy/BlockHandleEngine.java:1`  
> Registry: `defeatedcrow.EHandle` (`DCsAppleMilk.handleEngine`)  
> ItemBlock: `なし`  
> TileEntity: `TileHandleEngine` (`TileEHandle`) (`src/main/java/mods/defeatedcrow/common/tile/energy/TileHandleEngine.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:171` `public static Block handleEngine;`
- **インスタンス生成**: `MaterialRegister.java:1097` `new BlockHandleEngine()` / `setBlockName("defeatedcrow.EHandle")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.handleEngine, "defeatedcrow.EHandle")` (`MaterialRegister.java:334`)
- **Material / Hardness / Resistance**: `Material.clay`, hardness `0.5F`, resistance `1.0F` (`BlockHandleEngine.java:1`)
...
- Tile: `TileHandleEngine` (`src/main/java/mods/defeatedcrow/common/tile/TileHandleEngine.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileHandleEngine.class, "TileEHandle")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelHandleEngine`使用。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelHandleEngine` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityHandleEngineRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
