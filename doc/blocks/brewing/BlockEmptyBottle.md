...
> Category: `brewing`  
> Source: `src/main/java/mods/defeatedcrow/common/block/brewing/BlockEmptyBottle.java:1`  
> Registry: `defeatedcrow.emptyBottle` (`DCsAppleMilk.emptyBottle`)  
> ItemBlock: `ItemEmptyBottle` (`src/main/java/mods/defeatedcrow/common/block/brewing/ItemEmptyBottle.java:1`)  
> TileEntity: `TileEmptyBottle` (`TileEmptyBottle`) (`src/main/java/mods/defeatedcrow/common/tile/TileEmptyBottle.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:185` `public static Block emptyBottle;`
- **インスタンス生成**: `MaterialRegister.java:845` `new BlockEmptyBottle()` / `setBlockName("defeatedcrow.emptyBottle")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.emptyBottle, ItemEmptyBottle.class, "defeatedcrow.emptyBottle")` (`MaterialRegister.java:367`)
- **Material / Hardness / Resistance**: `Material.circuits`, hardness `0.2F`, resistance `1.0F` (`BlockEmptyBottle.java:1`)
...
- Tile: `TileEmptyBottle` (`src/main/java/mods/defeatedcrow/common/tile/TileEmptyBottle.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileEmptyBottle.class, "TileEmptyBottle")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelEmptyBottle`使用。
- ItemBlock: `ItemEmptyBottle` (`src/main/java/mods/defeatedcrow/common/block/brewing/ItemEmptyBottle.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelEmptyBottle` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityEmptyBottleRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
