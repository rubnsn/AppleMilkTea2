...
> Category: `brewing`  
> Source: `src/main/java/mods/defeatedcrow/common/block/brewing/BlockBarrel.java:1`  
> Registry: `defeatedcrow.blockBarrel` (`DCsAppleMilk.barrel`)  
> ItemBlock: `ItemAppliance` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemAppliance.java:1`)  
> TileEntity: `TileBrewingBarrel` (`TileBarrel`) (`src/main/java/mods/defeatedcrow/common/tile/TileBrewingBarrel.java:1`)  
> CreativeTab: `applemilk`

## 概要
醸造・瓶・樽ブロック。液体醸造や瓶詰めを担う。メタデータで内容物・残量を管理。

## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:188` `public static Block barrel;`
- **インスタンス生成**: `MaterialRegister.java:855` `new BlockBarrel()` / `setBlockName("defeatedcrow.blockBarrel")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.barrel, ItemAppliance.class, "defeatedcrow.blockBarrel")` (`MaterialRegister.java:370`)
- **Material / Hardness / Resistance**: `Material.wood`, hardness `0.3F`, resistance `` (`BlockBarrel.java:1`)
...
|---|---|---|---|---|
| `canSilkHarvest` | `public boolean canSilkHarvest(World world, EntityPlayer player, int x, int y, int z, int metadata) {` | `Block` オーバーライド / 内部呼出 | `canSilkHarvest` の1.7.10実装 | 1.12+: `canSilkHarvest(World, BlockPos, IBlockState, EntityPlayer)`。 |
...
- Tile: `TileBrewingBarrel` (`src/main/java/mods/defeatedcrow/common/tile/TileBrewingBarrel.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileBrewingBarrel.class, "TileBarrel")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelBarrel`使用。
...
## レシピ / ワールド生成 / その他連携
- 醸造: `TileBrewingBarrel`で液体醸造ロジック。`RegisterMakerRecipe.registerBrewing`でレシピ管理。
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelBarrel` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityBarrelRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
