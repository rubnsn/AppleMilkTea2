...
> Category: `brewing`  
> Source: `src/main/java/mods/defeatedcrow/common/block/brewing/BlockCordial.java:1`  
> Registry: `defeatedcrow.blockCordial` (`DCsAppleMilk.cordial`)  
> ItemBlock: `ItemBlockCordial` (`src/main/java/mods/defeatedcrow/common/block/brewing/ItemBlockCordial.java:1`)  
> TileEntity: `TileCordial` (`TileCordial`) (`src/main/java/mods/defeatedcrow/common/tile/TileCordial.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:187` `public static Block cordial;`
- **インスタンス生成**: `MaterialRegister.java:853` `new BlockCordial()` / `setBlockName("defeatedcrow.blockCordial")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.cordial, ItemBlockCordial.class, "defeatedcrow.blockCordial")` (`MaterialRegister.java:369`)
- **Material / Hardness / Resistance**: `Material.glass`, hardness `0.2F`, resistance `` (`BlockCordial.java:1`)
...
- Tile: `TileCordial` (`src/main/java/mods/defeatedcrow/common/tile/TileCordial.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileCordial.class, "TileCordial")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelCordial`使用。
- ItemBlock: `ItemBlockCordial` (`src/main/java/mods/defeatedcrow/common/block/brewing/ItemBlockCordial.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelCordial` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityCordialRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
