...
> Category: `brewing`  
> Source: `src/main/java/mods/defeatedcrow/common/block/brewing/BlockLargeBottle.java:1`  
> Registry: `defeatedcrow.largeBottle` (`DCsAppleMilk.largeBottle`)  
> ItemBlock: `ItemBlockBottle` (`src/main/java/mods/defeatedcrow/common/block/brewing/ItemBlockBottle.java:1`)  
> TileEntity: `TileLargeBottle` (`TileLargeBottle`) (`src/main/java/mods/defeatedcrow/common/tile/TileLargeBottle.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:186` `public static Block largeBottle;`
- **インスタンス生成**: `MaterialRegister.java:848` `new BlockLargeBottle()` / `setBlockName("defeatedcrow.largeBottle")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.largeBottle, ItemBlockBottle.class, "defeatedcrow.largeBottle")` (`MaterialRegister.java:368`)
- **Material / Hardness / Resistance**: `Material.circuits`, hardness `0.2F`, resistance `1.0F` (`BlockLargeBottle.java:1`)
- **CreativeTab**: `DCsAppleMilk.applemilk`
- **メタデータ**: `metadata & 15 = 種類`, `(metadata>>4)&7 = 残量`で管理。
...
- Tile: `TileLargeBottle` (`src/main/java/mods/defeatedcrow/common/tile/TileLargeBottle.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileLargeBottle.class, "TileLargeBottle")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelLargeBottle`使用。
- ItemBlock: `ItemBlockBottle` (`src/main/java/mods/defeatedcrow/common/block/brewing/ItemBlockBottle.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelLargeBottle` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityLargeBottleRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
