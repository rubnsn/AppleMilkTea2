...
> Category: `appliance`  
> Source: `src/main/java/mods/defeatedcrow/common/block/appliance/BlockTeaMakerBlack.java:1`  
> Registry: `defeatedcrow.teaMakerBlack` (`DCsAppleMilk.teaMakerBlack`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:154` `public static Block teaMakerBlack;`
- **インスタンス生成**: `MaterialRegister.java:1042` `new BlockTeaMakerBlack()` / `setBlockName("defeatedcrow.teaMakerBlack")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.teaMakerBlack, ItemAppliance.class, "defeatedcrow.teaMakerBlack")` (`MaterialRegister.java:318`)
- **Material / Hardness / Resistance**: `Material.unknown`, hardness `1.0F`, resistance `6000000.0F` (`BlockTeaMakerBlack.java:1`)
- **CreativeTab**: `DCsAppleMilk.applemilk`
- **メタデータ**: メタデータでの種類分岐あり。`getSubBlocks`/`getIcon`で分岐。詳細は`ItemAPI`または`Block`内の`boxType`配列等を参照。

## 継承・インターフェース
- 継承: `BlockTeaMakerNext`
- 実装: `なし`（または `IShearable` 等は個別クラスで確認）

## プロパティ / 状態
- `isOpaqueCube() -> false` / `renderAsNormalBlock() -> false` : 透過・特殊レンダー
- `getRenderType() -> DCsAppleMilk.modelTeaMakerBlack` (`DCsAppleMilk.java: - `, `ClientProxy.registerRenderers()`でISBRH登録)
...
|---|---|---|---|---|
| `canEntityDestroy` | `public boolean canEntityDestroy(IBlockAccess world, int x, int y, int z, Entity entity) {` | `Block` オーバーライド / 内部呼出 | `canEntityDestroy` の1.7.10実装 | 要確認: 1.7.10→1.12.2でシグネチャ変更の可能性あり。`World, int x,y,z`→`World, BlockPos, IBlockState`、`IIcon`→JSONモデル等のパターンに該当するか確認。 |
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelTeaMakerBlack` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
