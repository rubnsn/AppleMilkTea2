...
> Category: `appliance`  
> Source: `src/main/java/mods/defeatedcrow/common/block/appliance/BlockFilledSoupPan.java:1`  
> Registry: `defeatedcrow.filledSoupPan` (`DCsAppleMilk.filledSoupPan`)  
> ItemBlock: `ItemFilledSoupPan` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemFilledSoupPan.java:1`)  
> TileEntity: `TileFilledSoupPan` (`TileFilledSoupPan`) (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileFilledSoupPan.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:158` `public static Block filledSoupPan;`
- **インスタンス生成**: `MaterialRegister.java:1055` `new BlockFilledSoupPan()` / `setBlockName("defeatedcrow.filledSoupPan")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.filledSoupPan, ItemFilledSoupPan.class, "defeatedcrow.filledSoupPan")` (`MaterialRegister.java:321`)
- **Material / Hardness / Resistance**: `Material.ground`, hardness `0.2F`, resistance `1.0F` (`BlockFilledSoupPan.java:1`)
...
- Tile: `TileFilledSoupPan` (`src/main/java/mods/defeatedcrow/common/tile/TileFilledSoupPan.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileFilledSoupPan.class, "TileFilledSoupPan")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelFilledSoupPan`使用。
- ItemBlock: `ItemFilledSoupPan` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemFilledSoupPan.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelFilledSoupPan` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityFilledSoupPanRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
