...
> Category: `appliance`  
> Source: `src/main/java/mods/defeatedcrow/common/block/appliance/BlockIceMaker.java:1`  
> Registry: `defeatedcrow.iceMaker` (`DCsAppleMilk.iceMaker`)  
> ItemBlock: `なし`  
> TileEntity: `TileIceMaker` (`TileIceMaker`) (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileIceMaker.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:156` `public static Block iceMaker;`
- **インスタンス生成**: `MaterialRegister.java:1060` `new BlockIceMaker()` / `setBlockName("defeatedcrow.iceMaker")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.iceMaker, "defeatedcrow.iceMaker")` (`MaterialRegister.java:322`)
- **Material / Hardness / Resistance**: `Material.ground`, hardness `2.0F`, resistance `2.0F` (`BlockIceMaker.java:1`)
...
- Tile: `TileIceMaker` (`src/main/java/mods/defeatedcrow/common/tile/TileIceMaker.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileIceMaker.class, "TileIceMaker")`
  - GUI: `guiIceMaker (ID 2) `ContainerIceMaker`/`GuiIceMaker`` (`CommonProxy.getServerGuiElement/getClientGuiElement:1`)
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelIceMaker`使用。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelIceMaker` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityIceMakerRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
