...
> Category: `appliance`  
> Source: `src/main/java/mods/defeatedcrow/common/block/appliance/BlockEvaporator.java:1`  
> Registry: `defeatedcrow.evaporator` (`DCsAppleMilk.evaporator`)  
> ItemBlock: `ItemMachineBlock` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemMachineBlock.java:1`)  
> TileEntity: `TileEvaporator` (`TileEvaporator`) (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileEvaporator.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:161` `public static Block evaporator;`
- **インスタンス生成**: `MaterialRegister.java:1069` `new BlockEvaporator()` / `setBlockName("defeatedcrow.evaporator")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.evaporator, ItemMachineBlock.class, "defeatedcrow.evaporator")` (`MaterialRegister.java:326`)
- **Material / Hardness / Resistance**: `Material.ground`, hardness `2.0F`, resistance `2.0F` (`BlockEvaporator.java:1`)
...
- Tile: `TileEvaporator` (`src/main/java/mods/defeatedcrow/common/tile/TileEvaporator.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileEvaporator.class, "TileEvaporator")`
  - GUI: `guiEvaporator (4) `ContainerEvaporator`/`GuiEvaporator`` (`CommonProxy.getServerGuiElement/getClientGuiElement:1`)
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelEvaporator`使用。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelEvaporator` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityEvaporatorRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
