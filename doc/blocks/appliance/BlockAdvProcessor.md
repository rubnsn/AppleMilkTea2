...
> Category: `appliance`  
> Source: `src/main/java/mods/defeatedcrow/common/block/appliance/BlockAdvProcessor.java:1`  
> Registry: `defeatedcrow.advProcessor` (`DCsAppleMilk.advProcessor`)  
> ItemBlock: `ItemMachineBlock` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemMachineBlock.java:1`)  
> TileEntity: `TileAdvProcessor` (`TileAdvProcessor`) (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileAdvProcessor.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:162` `public static Block advProcessor;`
- **インスタンス生成**: `MaterialRegister.java:1066` `new BlockAdvProcessor()` / `setBlockName("defeatedcrow.advProcessor")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.advProcessor, ItemMachineBlock.class, "defeatedcrow.advProcessor")` (`MaterialRegister.java:325`)
- **Material / Hardness / Resistance**: `Material.ground`, hardness `2.0F`, resistance `2.0F` (`BlockAdvProcessor.java:1`)
...
- Tile: `TileAdvProcessor` (`src/main/java/mods/defeatedcrow/common/tile/TileAdvProcessor.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileAdvProcessor.class, "TileAdvProcessor")`
  - GUI: `guiAdvProcessor (5) `ContainerAdvProcessor`/`GuiAdvProcessor`` (`CommonProxy.getServerGuiElement/getClientGuiElement:1`)
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelAdvProcessor`使用。
- ItemBlock: `ItemMachineBlock` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemMachineBlock.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelAdvProcessor` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityAdvProcessorRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
