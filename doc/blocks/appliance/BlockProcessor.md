...
> Category: `appliance`  
> Source: `src/main/java/mods/defeatedcrow/common/block/appliance/BlockProcessor.java:1`  
> Registry: `defeatedcrow.processor` (`DCsAppleMilk.processor`)  
> ItemBlock: `ItemMachineBlock` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemMachineBlock.java:1`)  
> TileEntity: `TileProcessor` (`TileProcessor`) (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileProcessor.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:160` `public static Block processor;`
- **インスタンス生成**: `MaterialRegister.java:1063` `new BlockProcessor()` / `setBlockName("defeatedcrow.processor")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.processor, ItemMachineBlock.class, "defeatedcrow.processor")` (`MaterialRegister.java:324`)
- **Material / Hardness / Resistance**: `Material.ground`, hardness `2.0F`, resistance `2.0F` (`BlockProcessor.java:1`)
...
- Tile: `TileProcessor` (`src/main/java/mods/defeatedcrow/common/tile/TileProcessor.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileProcessor.class, "TileProcessor")`
  - GUI: `guiProcessor (3) `ContainerProcessor`/`GuiProcessor`` (`CommonProxy.getServerGuiElement/getClientGuiElement:1`)
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelProcessor`使用。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelProcessor` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityProcessorRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
