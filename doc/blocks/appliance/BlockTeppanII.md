...
> Category: `appliance`  
> Source: `src/main/java/mods/defeatedcrow/common/block/appliance/BlockTeppanII.java:1`  
> Registry: `defeatedcrow.teppanII` (`DCsAppleMilk.teppanII`)  
> ItemBlock: `ItemAppliance` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemAppliance.java:1`)  
> TileEntity: `TileTeppanII` (`TileTeppanII`) (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileTeppanII.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:159` `public static Block teppanII;`
- **インスタンス生成**: `MaterialRegister.java:1057` `new BlockTeppanII()` / `setBlockName("defeatedcrow.teppanII")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.teppanII, ItemAppliance.class, "defeatedcrow.teppanII")` (`MaterialRegister.java:323`)
- **Material / Hardness / Resistance**: `Material.iron`, hardness `3.0F`, resistance `10.0F` (`BlockTeppanII.java:1`)
...
- Tile: `TileTeppanII` (`src/main/java/mods/defeatedcrow/common/tile/TileTeppanII.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileTeppanII.class, "TileTeppanII")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelTeppanII`使用。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelTeppanII` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityTeppanIIRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
