...
> Category: `appliance`  
> Source: `src/main/java/mods/defeatedcrow/common/block/appliance/BlockTeaMakerNext.java:1`  
> Registry: `defeatedcrow.teaMakerNext` (`DCsAppleMilk.teaMakerNext`)  
> ItemBlock: `ItemAppliance` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemAppliance.java:1`)  
> TileEntity: `TileMakerNext` (`TileMakerNext`) (`src/main/java/mods/defeatedcrow/common/tile/appliance/TileMakerNext.java:1`)  
> CreativeTab: `applemilk`

## 概要
調理・加工装置ブロック。`BlockContainer`継承でTileEntityを持ち、GUI/レシピ処理を行う。

## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:153` `public static Block teaMakerNext;`
- **インスタンス生成**: `MaterialRegister.java:1039` `new BlockTeaMakerNext()` / `setBlockName("defeatedcrow.teaMakerNext")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.teaMakerNext, ItemAppliance.class, "defeatedcrow.teaMakerNext")` (`MaterialRegister.java:317`)
- **Material / Hardness / Resistance**: `Material.ground`, hardness `0.2F`, resistance `1.0F` (`BlockTeaMakerNext.java:1`)
...
| `getComparatorInputOverride` | `public int getComparatorInputOverride(World world, int i, int j, int k, int l) {` | コンパレーター出力値 (`Tile.remain`等) | `getComparatorInputOverride` の1.7.10実装 | 1.12+: `getComparatorInputOverride(IBlockState, World, BlockPos)`。`world.getTileEntity(pos)`維持。 |
...
| `hasComparatorInputOverride` | `public boolean hasComparatorInputOverride() {` | コンパレーター入力有無 | `hasComparatorInputOverride` の1.7.10実装 | 維持。1.12+: `hasComparatorInputOverride(IBlockState)`。 |
...
- Tile: `TileMakerNext` (`src/main/java/mods/defeatedcrow/common/tile/TileMakerNext.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileMakerNext.class, "TileMakerNext")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelTeaMakerNext`使用。
- ItemBlock: `ItemAppliance` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemAppliance.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- レシピ: `RegisterMakerRecipe.registerTea/registerIce/registerPan/registerPlate/registerProcessor/registerEvaporator` 等でレシピ登録。`RecipeRegisterManager`経由。
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火
- Achievement: `AchievementRegister.getTea/getAppleMilkTea`等を`triggerAchievement`で付与

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelTeaMakerNext` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityTeaMakerNextRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
