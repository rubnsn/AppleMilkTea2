...
> Category: `appliance`  
> Source: `src/main/java/mods/defeatedcrow/common/block/appliance/BlockEmptyPanG.java:1`  
> Registry: `defeatedcrow.emptyPanG` (`DCsAppleMilk.emptyPanGaiden`)  
> ItemBlock: `ItemPanG` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemPanG.java:1`)  
> TileEntity: `TilePanG` (`TilePanG`) (`src/main/java/mods/defeatedcrow/common/tile/appliance/TilePanG.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:157` `public static Block emptyPanGaiden;`
- **インスタンス生成**: `MaterialRegister.java:1052` `new BlockEmptyPanG()` / `setBlockName("defeatedcrow.emptyPanG")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.emptyPanGaiden, ItemPanG.class, "defeatedcrow.emptyPanG")` (`MaterialRegister.java:320`)
- **Material / Hardness / Resistance**: `Material.ground`, hardness `0.2F`, resistance `1.0F` (`BlockEmptyPanG.java:1`)
...
- Tile: `TilePanG` (`src/main/java/mods/defeatedcrow/common/tile/TilePanG.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TilePanG.class, "TilePanG")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelEmptyPanG`使用。
- ItemBlock: `ItemPanG` (`src/main/java/mods/defeatedcrow/common/block/appliance/ItemPanG.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelEmptyPanG` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityEmptyPanGRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
