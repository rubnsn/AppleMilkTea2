...
> Category: `plants`  
> Source: `src/main/java/mods/defeatedcrow/common/block/plants/BlockSaplingTea.java:1`  
> Registry: `defeatedcrow.saplingTea` (`DCsAppleMilk.saplingTea`)  
> ItemBlock: `ItemTeaSapling` (`src/main/java/mods/defeatedcrow/common/block/plants/ItemTeaSapling.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:212` `public static Block saplingTea;`
- **インスタンス生成**: `MaterialRegister.java:988` `new BlockSaplingTea()` / `setBlockName("defeatedcrow.saplingTea")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.saplingTea, ItemTeaSapling.class, "defeatedcrow.saplingTea")` (`MaterialRegister.java:357`)
- **Material / Hardness / Resistance**: `Material.plants`, hardness ``, resistance `` (`BlockSaplingTea.java:1`)
...
|---|---|---|---|---|
| `canBlockStay` | `public boolean canBlockStay(World par1World, int par2, int par3, int par4) {` | `Block` オーバーライド / 内部呼出 | `canBlockStay` の1.7.10実装 | 要確認: 1.7.10→1.12.2でシグネチャ変更の可能性あり。`World, int x,y,z`→`World, BlockPos, IBlockState`、`IIcon`→JSONモデル等のパターンに該当するか確認。 |
...
| `getPlant` | `public Block getPlant(IBlockAccess world, int x, int y, int z) {` | `Block` オーバーライド / 内部呼出 | `getPlant` の1.7.10実装 | 維持。 |
| `getPlantMetadata` | `public int getPlantMetadata(IBlockAccess world, int x, int y, int z) {` | `Block` オーバーライド / 内部呼出 | `getPlantMetadata` の1.7.10実装 | 1.12+で`getPlant(IBlockAccess, BlockPos)`のみ。メタは`IBlockState`へ。 |
| `getPlantType` | `public EnumPlantType getPlantType(IBlockAccess world, int x, int y, int z) {` | `IPlantable`植物タイプ | `getPlantType` の1.7.10実装 | 維持。`EnumPlantType`はそのまま。`IPlantable`継続。 |
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemTeaSapling` (`src/main/java/mods/defeatedcrow/common/block/plants/ItemTeaSapling.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelSaplingTea` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
