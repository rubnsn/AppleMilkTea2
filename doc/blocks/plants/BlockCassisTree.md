...
> Category: `plants`  
> Source: `src/main/java/mods/defeatedcrow/common/block/plants/BlockCassisTree.java:1`  
> Registry: `defeatedcrow.cassisTree` (`DCsAppleMilk.cassisTree`)  
> ItemBlock: `ItemCassisTree` (`src/main/java/mods/defeatedcrow/common/block/plants/ItemCassisTree.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:214` `public static Block cassisTree;`
- **インスタンス生成**: `MaterialRegister.java:1003` `new BlockCassisTree()` / `setBlockName("defeatedcrow.cassisTree")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.cassisTree, ItemCassisTree.class, "defeatedcrow.cassisTree")` (`MaterialRegister.java:359`)
- **Material / Hardness / Resistance**: `Material.wood`, hardness `0.1F`, resistance `` (`BlockCassisTree.java:1`)
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemCassisTree` (`src/main/java/mods/defeatedcrow/common/block/plants/ItemCassisTree.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelCassisTree` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
