...
> Category: `plants`  
> Source: `src/main/java/mods/defeatedcrow/common/block/plants/BlockTeaTree.java:1`  
> Registry: `defeatedcrow.teaTree` (`DCsAppleMilk.teaTree`)  
> ItemBlock: `ItemTeaTree` (`src/main/java/mods/defeatedcrow/common/block/plants/ItemTeaTree.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:213` `public static Block teaTree;`
- **インスタンス生成**: `MaterialRegister.java:991` `new BlockTeaTree()` / `setBlockName("defeatedcrow.teaTree")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.teaTree, ItemTeaTree.class, "defeatedcrow.teaTree")` (`MaterialRegister.java:358`)
- **Material / Hardness / Resistance**: `Material.wood`, hardness `0.1F`, resistance `` (`BlockTeaTree.java:1`)
- **CreativeTab**: `DCsAppleMilk.applemilk`
- **メタデータ**: `0:成長前, 1:成長後`で収穫可否を判定。`updateTick`でランダム成長。

## 継承・インターフェース
- 継承: `Block`
- 実装: `IShearable, IPlantable, IRightClickHarvestable`
...
|---|---|---|---|---|
| `canSilkHarvest` | `protected boolean canSilkHarvest() {` | `Block` オーバーライド / 内部呼出 | `canSilkHarvest` の1.7.10実装 | 1.12+: `canSilkHarvest(World, BlockPos, IBlockState, EntityPlayer)`。 |
| `damageDropped` | `damageDropped(...)` | ドロップ時のメタ決定 | `damageDropped` の1.7.10実装 | 1.12+: `damageDropped(IBlockState)`または`getMetaFromState`経由。`&7`等のマスクは`PropertyInteger`へ。 |
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemTeaTree` (`src/main/java/mods/defeatedcrow/common/block/plants/ItemTeaTree.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelTeaTree` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
