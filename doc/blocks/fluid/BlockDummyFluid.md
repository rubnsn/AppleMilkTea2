...
> Category: `fluid`  
> Source: `src/main/java/mods/defeatedcrow/common/fluid/BlockDummyFluid.java:1`  
> Registry: `defeatedcrow.blockDummyAlcohol` (`DCsAppleMilk.blockDummyAlcohol`)  
> ItemBlock: `ItemDummyFluid` (`src/main/java/mods/defeatedcrow/common/fluid/ItemDummyFluid.java:1`)  
> TileEntity: `なし`  
> CreativeTab: `なし`
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:339` `public static Block blockDummyAlcohol;`
- **インスタンス生成**: `MaterialRegister.java:864` `new BlockDummyFluid()` / `setBlockName("defeatedcrow.blockDummyAlcohol")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.blockDummyAlcohol, ItemDummyFluid.class, "defeatedcrow.blockDummyAlcohol")` (`MaterialRegister.java:372`)
- **Material / Hardness / Resistance**: `Material.water`, hardness ``, resistance `` (`BlockDummyFluid.java:1`)
...
- `getRenderType() -> DCsAppleMilk.modelDummyFluid` (`DCsAppleMilk.java: - `, `ClientProxy.registerRenderers()`でISBRH登録)
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemDummyFluid` (`src/main/java/mods/defeatedcrow/common/fluid/ItemDummyFluid.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelDummyFluid` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
