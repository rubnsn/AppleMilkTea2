...
> Category: `container`  
> Source: `src/main/java/mods/defeatedcrow/common/block/container/BlockMushBox.java:1`  
> Registry: `defeatedcrow.mushroomBox` (`DCsAppleMilk.mushroomBox`)  
> ItemBlock: `ItemMushBox` (`src/main/java/mods/defeatedcrow/common/block/container/ItemMushBox.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:197` `public static Block mushroomBox;`
- **インスタンス生成**: `MaterialRegister.java:893` `new BlockMushBox()` / `setBlockName("defeatedcrow.mushroomBox")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.mushroomBox, ItemMushBox.class, "defeatedcrow.mushroomBox")` (`MaterialRegister.java:389`)
- **Material / Hardness / Resistance**: `Material.wood`, hardness ``, resistance `` (`BlockMushBox.java:1`)
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemMushBox` (`src/main/java/mods/defeatedcrow/common/block/container/ItemMushBox.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelMushBox` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
