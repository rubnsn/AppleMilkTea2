...
> Category: `container`  
> Source: `src/main/java/mods/defeatedcrow/common/block/container/BlockWipeBox.java:1`  
> Registry: `defeatedcrow.wipeBox` (`DCsAppleMilk.wipeBox`)  
> ItemBlock: `ItemWipeBox` (`src/main/java/mods/defeatedcrow/common/block/container/ItemWipeBox.java:1`)  
> TileEntity: `TileWipeBox` (`TileWipeBox`) (`src/main/java/mods/defeatedcrow/common/tile/TileWipeBox.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:199` `public static Block wipeBox;`
- **インスタンス生成**: `MaterialRegister.java:905` `new BlockWipeBox()` / `setBlockName("defeatedcrow.wipeBox")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.wipeBox, ItemWipeBox.class, "defeatedcrow.wipeBox")` (`MaterialRegister.java:392`)
- **Material / Hardness / Resistance**: `Material.cloth`, hardness `0.2F`, resistance `1.0F` (`BlockWipeBox.java:1`)
...
- Tile: `TileWipeBox` (`src/main/java/mods/defeatedcrow/common/tile/TileWipeBox.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileWipeBox.class, "TileWipeBox")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelWipeBox`使用。
- ItemBlock: `ItemWipeBox` (`src/main/java/mods/defeatedcrow/common/block/container/ItemWipeBox.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelWipeBox` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityWipeBoxRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
