...
> Category: `container`  
> Source: `src/main/java/mods/defeatedcrow/common/block/container/BlockAppleBox.java:1`  
> Registry: `defeatedcrow.AppleBox` (`DCsAppleMilk.appleBox`)  
> ItemBlock: `ItemAppleBox` (`src/main/java/mods/defeatedcrow/common/block/container/ItemAppleBox.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:191` `public static Block appleBox;`
- **インスタンス生成**: `MaterialRegister.java:873` `new BlockAppleBox()` / `setBlockName("defeatedcrow.AppleBox")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.appleBox, ItemAppleBox.class, "defeatedcrow.AppleBox")` (`MaterialRegister.java:338`)
- **Material / Hardness / Resistance**: `Material.wood`, hardness `0.1F`, resistance `` (`BlockAppleBox.java:1`)
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemAppleBox` (`src/main/java/mods/defeatedcrow/common/block/container/ItemAppleBox.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelAppleBox` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
