...
> Category: `container`  
> Source: `src/main/java/mods/defeatedcrow/common/block/container/BlockWoodBox.java:1`  
> Registry: `defeatedcrow.WoodBox` (`DCsAppleMilk.woodBox`)  
> ItemBlock: `ItemWoodBox` (`src/main/java/mods/defeatedcrow/common/block/container/ItemWoodBox.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:190` `public static Block woodBox;`
- **インスタンス生成**: `MaterialRegister.java:870` `new BlockWoodBox()` / `setBlockName("defeatedcrow.WoodBox")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.woodBox, ItemWoodBox.class, "defeatedcrow.WoodBox")` (`MaterialRegister.java:337`)
- **Material / Hardness / Resistance**: `Material.wood`, hardness `0.1F`, resistance `` (`BlockWoodBox.java:1`)
- **CreativeTab**: `DCsAppleMilk.applemilkContainer`
- **メタデータ**: `0-12`で樹種分岐（oak, spruce, birch, jungle, rubber...）。`getSubBlocks`で13種をCreativeTabに登録。
...
- `getRenderType() -> DCsAppleMilk.modelWoodBox` (`DCsAppleMilk.java: - `, `ClientProxy.registerRenderers()`でISBRH登録)
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemWoodBox` (`src/main/java/mods/defeatedcrow/common/block/container/ItemWoodBox.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelWoodBox` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
