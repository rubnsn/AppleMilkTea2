...
> Category: `container`  
> Source: `src/main/java/mods/defeatedcrow/common/block/container/BlockFlowerPot.java:1`  
> Registry: `defeatedcrow.flowerPot` (`DCsAppleMilk.flowerPot`)  
> ItemBlock: `ItemFlowerPot` (`src/main/java/mods/defeatedcrow/common/block/container/ItemFlowerPot.java:1`)  
> TileEntity: `TileFlowerPot` (`TileFlowerPot`) (`src/main/java/mods/defeatedcrow/common/tile/TileFlowerPot.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:203` `public static Block flowerPot;`
- **インスタンス生成**: `MaterialRegister.java:910` `new BlockFlowerPot()` / `setBlockName("defeatedcrow.flowerPot")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.flowerPot, ItemFlowerPot.class, "defeatedcrow.flowerPot")` (`MaterialRegister.java:348`)
- **Material / Hardness / Resistance**: `Material.ground`, hardness `0.2F`, resistance `1.0F` (`BlockFlowerPot.java:1`)
...
- Tile: `TileFlowerPot` (`src/main/java/mods/defeatedcrow/common/tile/TileFlowerPot.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileFlowerPot.class, "TileFlowerPot")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelFlowerPot`使用。
- ItemBlock: `ItemFlowerPot` (`src/main/java/mods/defeatedcrow/common/block/container/ItemFlowerPot.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelFlowerPot` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityFlowerPotRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
