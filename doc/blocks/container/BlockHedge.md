...
> Category: `container`  
> Source: `src/main/java/mods/defeatedcrow/common/block/container/BlockHedge.java:1`  
> Registry: `defeatedcrow.hedge` (`DCsAppleMilk.hedge`)  
> ItemBlock: `ItemHedge` (`src/main/java/mods/defeatedcrow/common/block/container/ItemHedge.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:206` `public static Block hedge;`
- **インスタンス生成**: `MaterialRegister.java:932` `new BlockHedge()` / `setBlockName("defeatedcrow.hedge")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.hedge, ItemHedge.class, "defeatedcrow.hedge")` (`MaterialRegister.java:350`)
- **Material / Hardness / Resistance**: `Material.wood`, hardness `0.1F`, resistance `` (`BlockHedge.java:1`)
...
| `onEntityCollidedWithBlock` | `public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity par5Entity) {` | エンティティ接触時 | `onEntityCollidedWithBlock` の1.7.10実装 | 1.12+: `onEntityCollision(World, BlockPos, IBlockState, Entity)` (1.16+ `onEntityCollision(BlockState, World, BlockPos, Entity)`)。 |
...
| `setBlockBoundsBasedOnState` | `public void setBlockBoundsBasedOnState(IBlockAccess world, int par2, int par3, int par4) {` | 状態依存のBounds設定 | `setBlockBoundsBasedOnState` の1.7.10実装 | 1.12+で`getBoundingBox(IBlockState, IBlockAccess, BlockPos)`へ。`setBlockBounds`は削除。AABBは`AxisAlignedBB`→`VoxelShape`へ。 |
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemHedge` (`src/main/java/mods/defeatedcrow/common/block/container/ItemHedge.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelHedge` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
