# BlockContainerWaterBottle

> Category: `container`  
> Source: `src/main/java/mods/defeatedcrow/common/block/container/BlockContainerWaterBottle.java:1`  
> Registry: `defeatedcrow.containerBottleW` (`DCsAppleMilk.containerWBottle`)  
> ItemBlock: `ItemContainerBase` (`src/main/java/mods/defeatedcrow/common/block/container/ItemContainerBase.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:208` `public static Block containerWBottle;`
- **インスタンス生成**: `MaterialRegister.java:913` `new BlockContainerWaterBottle()` / `setBlockName("defeatedcrow.containerBottleW")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.containerWBottle, ItemContainerBase.class, "defeatedcrow.containerBottleW")` (`MaterialRegister.java:352`)
- **Material / Hardness / Resistance**: `Material.unknown`, hardness ``, resistance `` (`BlockContainerWaterBottle.java:1`)
...
| `returnItem` | `public ItemStack returnItem() {` | `Block` オーバーライド / 内部呼出 | `returnItem` の1.7.10実装 | BlockContainerBase 独自。`ItemStack returnItem()`は維持。`Items.xxx`は`DeferredRegister`参照へ。 |

## TileEntity / ItemBlock 連携
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemContainerBase` (`src/main/java/mods/defeatedcrow/common/block/container/ItemContainerBase.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelContainerWaterBottle` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
