...
> Category: `container`  
> Source: `src/main/java/mods/defeatedcrow/common/block/container/BlockFlowerVase.java:1`  
> Registry: `defeatedcrow.flowerVase` (`DCsAppleMilk.flowerBase`)  
> ItemBlock: `ItemFlowerVase` (`src/main/java/mods/defeatedcrow/common/block/container/ItemFlowerVase.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:205` `public static Block flowerBase;`
- **インスタンス生成**: `MaterialRegister.java:929` `new BlockFlowerVase()` / `setBlockName("defeatedcrow.flowerVase")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.flowerBase, ItemFlowerVase.class, "defeatedcrow.flowerVase")` (`MaterialRegister.java:349`)
- **Material / Hardness / Resistance**: `Material.wood`, hardness `0.1F`, resistance `` (`BlockFlowerVase.java:1`)
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemFlowerVase` (`src/main/java/mods/defeatedcrow/common/block/container/ItemFlowerVase.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelFlowerVase` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
