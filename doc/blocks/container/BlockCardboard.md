...
> Category: `container`  
> Source: `src/main/java/mods/defeatedcrow/common/block/container/BlockCardboard.java:1`  
> Registry: `defeatedcrow.cardboardBox` (`DCsAppleMilk.cardboard`)  
> ItemBlock: `ItemCardboard` (`src/main/java/mods/defeatedcrow/common/block/container/ItemCardboard.java:1`)  
> TileEntity: `TileCardBoard` (`TileCardboard`) (`src/main/java/mods/defeatedcrow/common/tile/TileCardBoard.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:193` `public static Block cardboard;`
- **インスタンス生成**: `MaterialRegister.java:879` `new BlockCardboard()` / `setBlockName("defeatedcrow.cardboardBox")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.cardboard, ItemCardboard.class, "defeatedcrow.cardboardBox")` (`MaterialRegister.java:340`)
- **Material / Hardness / Resistance**: `Material.wood`, hardness `0.1F`, resistance `` (`BlockCardboard.java:1`)
- **CreativeTab**: `DCsAppleMilk.applemilkContainer`
- **メタデータ**: `meta &7 = 内容物`, `meta &8 = 向きフラグ`。Tileで`directionByte`保持。
...
- Tile: `TileCardBoard` (`src/main/java/mods/defeatedcrow/common/tile/TileCardBoard.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileCardBoard.class, "TileCardboard")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelCardboard`使用。
- ItemBlock: `ItemCardboard` (`src/main/java/mods/defeatedcrow/common/block/container/ItemCardboard.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelCardboard` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityCardboardRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
