# BlockChalcedonyLampOp

> Category: `chalcedony`  
> Source: `src/main/java/mods/defeatedcrow/common/block/BlockChalcedonyLampOp.java:1`  
> Registry: `defeatedcrow.chalcedonyLampOp` (`DCsAppleMilk.cLampOpaque`)  
> ItemBlock: `ItemCLampOp` (`src/main/java/mods/defeatedcrow/common/block/ItemCLampOp.java:1`)  
> TileEntity: `なし`  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:233` `public static Block cLampOpaque;`
- **インスタンス生成**: `MaterialRegister.java:960` `new BlockChalcedonyLampOp()` / `setBlockName("defeatedcrow.chalcedonyLampOp")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.cLampOpaque, ItemCLampOp.class, "defeatedcrow.chalcedonyLampOp")` (`MaterialRegister.java:380`)
- **Material / Hardness / Resistance**: `Material.glass`, hardness `0.5F`, resistance `1.0F` (`BlockChalcedonyLampOp.java:1`)
...
- Tile: `なし`（`Block`単体）
- ItemBlock: `ItemCLampOp` (`src/main/java/mods/defeatedcrow/common/block/ItemCLampOp.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelChalcedonyLampOp` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
