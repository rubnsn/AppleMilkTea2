...
> Category: `chalcedony`  
> Source: `src/main/java/mods/defeatedcrow/common/block/BlockRotaryDial.java:1`  
> Registry: `defeatedcrow.rotaryDial` (`DCsAppleMilk.rotaryDial`)  
> ItemBlock: `なし`  
> TileEntity: `TileRotaryDial` (`TileRotaryDial`) (`src/main/java/mods/defeatedcrow/common/tile/TileRotaryDial.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:231` `public static Block rotaryDial;`
- **インスタンス生成**: `MaterialRegister.java:964` `new BlockRotaryDial()` / `setBlockName("defeatedcrow.rotaryDial")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.rotaryDial, "defeatedcrow.rotaryDial")` (`MaterialRegister.java:394`)
- **Material / Hardness / Resistance**: `Material.glass`, hardness `0.2F`, resistance `1.0F` (`BlockRotaryDial.java:1`)
...
- Tile: `TileRotaryDial` (`src/main/java/mods/defeatedcrow/common/tile/TileRotaryDial.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileRotaryDial.class, "TileRotaryDial")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelRotaryDial`使用。
...
## レンダー / モデル
- Render ID: `DCsAppleMilk.modelRotaryDial` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityRotaryDialRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
