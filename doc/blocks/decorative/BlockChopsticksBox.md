...
> Category: `decorative`  
> Source: `src/main/java/mods/defeatedcrow/common/block/BlockChopsticksBox.java:1`  
> Registry: `defeatedcrow.chopsticksBox` (`DCsAppleMilk.chopsticksBox`)  
> ItemBlock: `ItemChopsticksBox` (`src/main/java/mods/defeatedcrow/common/block/ItemChopsticksBox.java:1`)  
> TileEntity: `TileChopsticksBox` (`TileChopsticksBox`) (`src/main/java/mods/defeatedcrow/common/tile/TileChopsticksBox.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:224` `public static Block chopsticksBox;`
- **インスタンス生成**: `MaterialRegister.java:944` `new BlockChopsticksBox()` / `setBlockName("defeatedcrow.chopsticksBox")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.chopsticksBox, ItemChopsticksBox.class, "defeatedcrow.chopsticksBox")` (`MaterialRegister.java:387`)
- **Material / Hardness / Resistance**: `Material.wood`, hardness `0.2F`, resistance `1.0F` (`BlockChopsticksBox.java:1`)
...
- Tile: `TileChopsticksBox` (`src/main/java/mods/defeatedcrow/common/tile/TileChopsticksBox.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileChopsticksBox.class, "TileChopsticksBox")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelChopsticksBox`使用。
- ItemBlock: `ItemChopsticksBox` (`src/main/java/mods/defeatedcrow/common/block/ItemChopsticksBox.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelChopsticksBox` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityChopsticksBoxRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
