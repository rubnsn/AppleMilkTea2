...
> Category: `decorative`  
> Source: `src/main/java/mods/defeatedcrow/common/block/BlockBasket.java:1`  
> Registry: `defeatedcrow.basket` (`DCsAppleMilk.Basket`)  
> ItemBlock: `ItemBreadBasket` (`src/main/java/mods/defeatedcrow/common/block/ItemBreadBasket.java:1`)  
> TileEntity: `TileBread` (`TileBread`) (`src/main/java/mods/defeatedcrow/common/tile/TileBread.java:1`)  
...
## 登録情報
- **フィールド定義**: `DCsAppleMilk.java:223` `public static Block Basket;`
- **インスタンス生成**: `MaterialRegister.java:941` `new BlockBasket()` / `setBlockName("defeatedcrow.basket")`
- **GameRegistry**: `GameRegistry.registerBlock(DCsAppleMilk.Basket, ItemBreadBasket.class, "defeatedcrow.basket")` (`MaterialRegister.java:385`)
- **Material / Hardness / Resistance**: `Material.ground`, hardness `0.2F`, resistance `1.0F` (`BlockBasket.java:1`)
...
- Tile: `TileBread` (`src/main/java/mods/defeatedcrow/common/tile/TileBread.java:1`) - `CommonProxy.registerTileEntity()` で `GameRegistry.registerTileEntity(TileBread.class, "TileBread")`
  - GUI: `なし`（Tileのみ。インベントリは`breakBlock`で散乱）
  - Renderer: `ClientProxy.registerRenderers()`でISBRH/TileEntityRenderer登録。`modelBasket`使用。
- ItemBlock: `ItemBreadBasket` (`src/main/java/mods/defeatedcrow/common/block/ItemBreadBasket.java:1`) - `getUnlocalizedName(ItemStack)` / `getMetadata()` / `placeBlockAt()` 等。メタ分岐の表示名対応。

## レシピ / ワールド生成 / その他連携
- Event: `AMTBlockRightClickEvent` / `PlantsClickEvent` / `TeamakerRightClickEvent` 等を `MinecraftForge.EVENT_BUS.post(event)` で発火

## レンダー / モデル
- Render ID: `DCsAppleMilk.modelBasket` (`DCsAppleMilk.java: - ` 初期化, `CommonProxy.getRenderID()` , `ClientProxy.registerRenderers()` で登録)
- Renderer: `TileEntityBasketRenderer` / `ISimpleBlockRenderingHandler` 実装 (`src/main/java/mods/defeatedcrow/client/model/tileentity/*`)
