...
public class TileEntityAlcoholCupRenderer implements BlockEntityRenderer<TileAlcoholCup> {
    ...
    public TileEntityAlcoholCupRenderer(BlockEntityRendererProvider.Context ctx) {
        ...
        this.model = new ModelAlcoholCup(ModelAlcoholCup.createBodyLayer().bakeRoot());
    }

    @Override
    public void render(TileAlcoholCup be, float partialTicks, PoseStack pose, MultiBufferSource buffers, int light, int overlay) {
        pose.pushPose();
