...
public class TileEntityEHandleRenderer implements BlockEntityRenderer<TileHandleEngine> {
    ...
    public TileEntityEHandleRenderer(BlockEntityRendererProvider.Context ctx) {
        ...
        this.model = new ModelHandleEngine(ModelHandleEngine.createBodyLayer().bakeRoot());
    }

    @Override
    public void render(TileHandleEngine be, float partialTicks, PoseStack pose, MultiBufferSource buffers, int light, int overlay) {
        pose.pushPose();
