...
public class TileEntityCrowdollRenderer implements BlockEntityRenderer<TileCrowDoll> {
    private final ModelCrowDoll model;
    private static final ResourceLocation TEX = new ResourceLocation("defeatedcrow", "textures/entity/crowdoll.png");

    public TileEntityCrowdollRenderer(BlockEntityRendererProvider.Context ctx) {
        // Use LayerDefinition bakeRoot directly for minimal implementation (no ModModelLayers)
        this.model = new ModelCrowDoll(ModelCrowDoll.createBodyLayer().bakeRoot());
    }

    @Override
    public void render(TileCrowDoll be, float partialTicks, PoseStack pose, MultiBufferSource buffers, int light, int overlay) {
        pose.pushPose();
