...
public class TileEntityProcessorRenderer implements BlockEntityRenderer<TileProcessor> {
    ...
    public TileEntityProcessorRenderer(BlockEntityRendererProvider.Context ctx) {
        // Use LayerDefinition bakeRoot directly for minimal implementation (no ModModelLayers)
        this.model = new ModelProcessor(ModelProcessor.createBodyLayer().bakeRoot());
    }

    @Override
    public void render(TileProcessor be, float partialTicks, PoseStack pose, MultiBufferSource buffers, int light, int overlay) {
        pose.pushPose();
