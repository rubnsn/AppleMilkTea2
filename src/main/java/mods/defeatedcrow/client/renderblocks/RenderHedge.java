...
public class RenderHedge implements ISimpleBlockRenderingHandler {
    ...
    public void renderInventoryBlock(Block block, int metadata, int modelID, RenderBlocks renderer) {
        ...
        if (block instanceof BlockHedge) hedge = (BlockHedge) block;
        else return;
        HedgeType type = hedge.getType(meta);
        if (type == HedgeType.LATTICE) this.baseIcon = block.getIcon(3, 0);
        else if (type == HedgeType.FENCE) this.baseIcon = block.getIcon(2, 0);
        else this.baseIcon = block.getIcon(1, 0);
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        ...
        if (modelId == this.getRenderId()) {
            ...
            boolean[] adj = { false, false, false, false }; // n, s, e, w
