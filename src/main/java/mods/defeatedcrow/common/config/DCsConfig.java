...
public class DCsConfig {
    ...
    public void config(Configuration cfg) {
        ...
        try {
            ...
            Property TeaTreeValue = cfg.get(
                "world setting",
                "Tea Tree Gen Probability",
                teaTreeGenValue,
                "Set the generation probability of tea tree.(1-20) Default value is 5.");
            Property ClamValue = cfg.get(
                "world setting",
                "Clam Gen Probability",
                clamChanceValue,
                "Set the generation probability and the increase probability of clam.(1-12) Default value is 5.");
            Property EXRecipe = cfg
                .get("setting", "Use Extra Recipe", useEXRecipe, "Add recipe for crafting tea tree sapling.");
            Property noTeaTree = cfg
                .get("world setting", "No Gen TeaTree", notGenTeaTree, "Not generating tea tree on overworld.");
            Property allowSlimeBall = cfg.get(
                "setting",
                "Allow Slimeball OreDic",
                allowSlimeBallDic,
                "Allow to add SlimeBall and Animalglue to Oredictionary.");
            Property noSteam = cfg.get(
                "render setting",
                "Not Render Foods Steam",
                noRenderFoodsSteam,
                "Not Render Steam on foods of this mod.");
            Property noFirestarter = cfg
                .get("setting", "Disable Fierstarter", disableFireSteater, "Disable recipe for crafting firestarter.");
            Property noClam = cfg
                .get("world setting", "Disable Clams", disableClam, "Not generating clams on overworld.");
            Property nowetGC = cfg
                .get("Setting", "No Weathering Container", noWetGContainer, "Not weathering Gunpowdercontainer.");
            Property cupTex = cfg.get(
                "render setting",
                "Set JPbowl Texture",
                setCupTexture,
                "Select the texture of the JP bowls." + "1:cherry flowers, 2:blue pattern, 3:white porcelain");
            Property useSummer = cfg.get(
                "render setting",
                "Use Summer Rendering",
                useSummerRender,
                "Use the Summer Rendering to the Cups.");
            Property teppannHard = cfg.get(
                "setting",
                "Teppann Hard Mode",
                teppannHardMode,
                "Enable time limit to get the food from the iron plate.");
            Property teppannTime = cfg.get(
                "setting",
                "Teppann Ready Time",
                teppannReadyTime,
                "Set the length of time limit to get the food from the iron plate.(min 1 - max 60)");
            Property teppannRandom = cfg.get(
                "setting",
                "Randomly Teppann Cooking Time",
                teppannRandomCookTime,
                "Enable randomly cooking time of iron plate.");
            Property entityMelon = cfg.get("entityid", "EntityIDCompressedMelon", entityIdMelon);
            Property explodeMelon = cfg
                .get("entity setting", "Enable Explode Melon", canExplodeMelon, "Allow the Compressed Melon explode.");
            Property cupStackSizeInt = cfg.get(
                "setting",
                "Cups Stack Size",
                cupStackSize,
                "Set stack seize of filled cups. Please choose from the 1/3/8.");
            Property melonBreak = cfg.get(
                "entity setting",
                "Melon not Break Block",
                melonBreakBlock,
                "Disable destruction by explosion of melon.");
            Property melonFear = cfg.get(
                "entity setting",
                "Silky Melon of Fear",
                fearMelon,
                "Silky Melon can destroy all blocks except blocks that coordinates Y = 1.");
            Property completeFear = cfg.get(
                "entity setting",
                "Silky Melon of the Complete Fear",
                completeFearMelon,
                "Silky Melon can destroy all blocks include bedrock, and force it to drop.");
            Property missileExplosion = cfg.get(
                "entity setting",
                "Dsiable Missile Explosion",
                disableMissileExplosion,
                "Disable explosion of missiles generated from the Fossil Scale.");
            Property safetyChoco = cfg.get(
                "setting",
                "Safety Chocolate Gift",
                safetyChocolate,
                "Disable explosion of the heartfelt chocolate gift.");
            Property infinityWipes = cfg.get(
                "setting",
                "Allow Infinity Wipes",
                allowInfinityWipes,
                "Allow the WipeBox generate a paper infinitely.");
            Property bonemealClams = cfg.get(
                "setting",
                "Allow Clam Fertilizer",
                bonemealClam,
                "Allow to use the clam and the clam container as fertilizer.");
            Property texPass = cfg.get(
                "render setting",
                "Set Texture Type Number",
                setAltTexturePass,
                "Select the texture type number." + "1:default(x16 tex), 2:use x32 tex");
            Property cafeRecipe = cfg
                .get("entity setting", "New Villager ID", villagerRecipeID, "Set the number of new villager ID.");
            Property edibleEntity = cfg.get(
                "render setting",
                "Enable Edible Entity",
                allowEdibleEntities,
                "Some food blocks are placed as a entity. If false, these are placed as a block. (just same as old version.)");
            Property princessChance = cfg.get(
                "setting",
                "Princess Chance Value",
                princessChanceValue,
                "Set the generation probability of princess clam. Default value is 5.");
            Property radenLimit = cfg.get(
                "setting",
                "Hard Mode Wind Charm",
                charmRemain,
                "Set a limit on the number of times to use the Raden Charm (Wind)." + BR
                    + "If you set 0, disable Hard Mode.");
            Property mobBlock = cfg.get(
                "setting",
                "Enable Mob Drop Container",
                enableMobBlock,
                "Enable to add some compression recipes of mob drop items." + BR
                    + "Rotten flesh, Bone, Spider eye, Ender parl, and Slime ball.");
            Property hardLeather = cfg.get(
                "setting",
                "Hard Mode Leather Recipe",
                hardLeatherRecipe,
                "Enable hard mode that make a leather from a rotten flesh.");
            Property batteryCycle = cfg.get(
                "setting",
                "Battery Update Cycle",
                batteryUpdate,
                "Set the update cycle tick of the device using the battery.");
            Property debug = cfg.get(
                "debug setting",
                "Debug Mode Pass",
                debugPass,
                "Input the password for starting in debug mode. This is only for developer.");
            Property noPvP = cfg.get(
                "setting",
                "PvP Prohibition Mode",
                PvPProhibitionMode,
                "Disable the damage caused by items of this MOD against player." + BR
                    + "(For example, Yuzu Gatling, Silky Melon, etc.)");
            Property cupScale = cfg.get(
                "render setting",
                "Set Drink Entity Scale",
                setCupScale,
                "Select the scale of drink entity (like a cup). please set 0.01F - 10.0F");
            Property cropBurn = cfg.get(
                "setting",
                "Yuzu Crop Burn In Device",
                yuzuCropBurn,
                "Yuzu crops can be burned in the chargeable appliance, and generate 80 charge.");

            Property dustDifP = cfg.get(
                "difficulty setting",
                "JawCrusher Dust Gen",
                dustDif,
                "Change difficulty of the JawCrusher recipe." + BR + "0:sweet 1:normal 2:bitter 3:hard");

            Property chargeDifP = cfg.get(
                "difficulty setting",
                "Battery Charge Gen",
                chargeDif,
                "Change difficulty of the battery charge amount." + BR + "0:sweet 1:normal 2:bitter 3:hard");

            Property exchangeDifP = cfg.get(
                "difficulty setting",
                "Exchange Rate of Charge",
                exchangeDif,
            ...
            Property altModR = cfg.get(
                "difficulty setting",
                "Another Mod Recipe",
                altModRecipe,
                "Enable Recipes added the another mod machines.");

            Property procDifP = cfg.get(
                "difficulty setting",
                "JawCrusher Recipe Difficulty",
                procDif,
                "Change difficulty of the JawCrusher recipe tier." + BR + "0:sweet 1:normal 2:bitter");

            Property useAltTeppan = cfg.get(
                "render setting",
                "Enable Alt Teppan Texture",
                useAltTeppanTex,
                "Enable Alternate Teppan Texture like a mesh grill.");

            Property warpKey = cfg.get(
                "setting",
                "Charm Warp Key Number",
                charmWarpKey,
