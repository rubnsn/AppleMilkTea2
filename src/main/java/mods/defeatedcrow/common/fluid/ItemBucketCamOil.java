 ...
 */
public class ItemBucketCamOil extends BucketItem {

    public ItemBucketCamOil(Supplier<? extends Fluid> fluid) {
        super(fluid, new Item.Properties().craftRemainder(Items.BUCKET).stacksTo(1));
    }

    // Legacy constructor for compat (block param removed)
    public ItemBucketCamOil(net.minecraft.world.level.block.Block block) {
        this(() -> (Fluid)ModFluids.CAMELLIA_OIL_SOURCE.get());
    }
}
