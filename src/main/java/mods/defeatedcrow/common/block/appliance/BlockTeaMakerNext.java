...
public class BlockTeaMakerNext extends BlockContainer {
    ...
    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer,
        ...
        } else if (tileItem != null && remain > 0) // 空ではない
        {
            ...
            if (item == Item.getItemFromBlock(DCsAppleMilk.emptyCup)) // 空マグで右クリックする処理を最優先に
            {
                    ...
                    if (output != null) {
                        ...
                        if (!par1World.isRemote) {
                            EntityItem drop = new EntityItem(
                                ...
                                new ItemStack(output.getItem(), 1, output.getItemDamage()));
                ...
                && DCsAppleMilk.SuccessLoadIC2
                && LoadIC2Plugin.IC2Mug != null
                && LoadIC2Plugin.IC2MugCoffee != null
                && itemstack.getItem() == LoadIC2Plugin.IC2Mug.getItem()) // IC2コーヒー
            {
                if (!par5EntityPlayer.capabilities.isCreativeMode && --itemstack.stackSize <= 0) {
                    par5EntityPlayer.inventory
                        .setInventorySlotContents(par5EntityPlayer.inventory.currentItem, (ItemStack) null);
            ...
            } else if (recipe != null) // 手持ちアイテムが登録済みの茶の材料のとき
            {
                ...
                if (tile.getItemStack()
                    .getItem() == Items.milk_bucket) // ティーメーカーが牛乳のみのとき
                {
    ...
    private boolean isEmptyCell(ItemStack itemstack) {
        ...
        if (itemstack.getItem() == Items.bucket) flag = true;
            ...
            && itemstack.getItem() == LoadIC2Plugin.IC2Cell.getItem()) flag = true;
        else flag = false;
