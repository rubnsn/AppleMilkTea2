...
public class BlockEmptyPanG extends Block {
    ...
    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer,
        ...
        } else if (itemstack.getItem() == Items.bowl)// 木のボウル：ノーマルのoutputを返し、remainを1減らす。
        {
            ...
            return true;
        } else if (!LoadBambooPlugin.getBasket()
            .isEmpty() && LoadBambooPlugin.isBasketItem(itemstack) >= 0)// 竹のボウル：和風のoutputを返し、remainを1減らす。
        {
            if (tile.getOutput() != null && tile.getRemainByte() > 0) {
                ItemStack ret = tile.getOutputJP()
                    .copy();
            ...
            if (this.onFurnace(par1World, par2, par3 - 1, par4) && tile.getItemStack() == null) {
                ...
                if (recipe != null) {
                    ...
                    if (par1World.isRemote) par5EntityPlayer.addChatMessage(
                ...
                if (!this.onFurnace(par1World, par2, par3 - 1, par4)) {
                    if (par1World.isRemote) par5EntityPlayer.addChatMessage(
                        new ChatComponentText(StatCollector.translateToLocal("dc.panMessage.noHeatSource")));
                    return true;
                } else if (tile.getItemStack() != null) {
                    if (par1World.isRemote) par5EntityPlayer.addChatMessage(
                        new ChatComponentText(StatCollector.translateToLocal("dc.panMessage.alreadyHasRecipe")));
                    return true;
                }
                return false;
            }
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        if (world.isRemote || !this.onFurnace(world, x, y - 1, z) || entity == null) return;
        TileEntity tile = world.getTileEntity(x, y, z);
        if (tile == null || !(tile instanceof TilePanG)) return;
    ...
    public void registerBlockTextures(BlockIconRegister par1BlockIconRegister) {
        this.boxTex = par1BlockIconRegister.registerIcon("defeatedcrow:porcelain");
        this.chainTex = par1BlockIconRegister.registerIcon(Util.getTexturePassNoAlt() + "chain");
    }
