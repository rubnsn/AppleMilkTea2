...
public class BlockTeppanII extends BlockContainer {
    ...
    public boolean isOvenMode(World world, int x, int y, int z) {
        ...
        if (world.canBlockSeeTheSky(x, y, z)) {
            for (int i = 0; i < 3; i++) {
                if (!world.isAirBlock(x, y + 1 + i, z) && world.getBlock(x, y + 1 + i, z)
    ...
    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer,
        ...
        if (DCsAppleMilk.debugMode) {
            ...
            if (rep != null && rep.getOutput() != null) {
                AMTLogger.debugInfo(
                    "equip item output : " + rep.getOutput()
                        .getDisplayName());
            }
        ...
        if (!par1World.isRemote && tep != null) {
            if (!tep.isOnHeatSource()) {
                par5EntityPlayer.addChatMessage(
                    new ChatComponentText(StatCollector.translateToLocal("dc.plateMessage.noHeatSource")));
                return true;
            }
                ...
                if (tep.canSetRecipe(item)) {
                    ...
                    if (recipe.useOvenRecipe() && !this.isOvenMode(par1World, par2, par3, par4)) {
                        if (par1World.isRemote) par5EntityPlayer.addChatMessage(
                            new ChatComponentText(StatCollector.translateToLocal("dc.plateMessage.needOvenMode")));
                    }

                    return true;
                } else {
                    if (par1World.isRemote) par5EntityPlayer.addChatMessage(
                        new ChatComponentText(StatCollector.translateToLocal("dc.plateMessage.noRecipe")));
                    return true;
                }
            }
