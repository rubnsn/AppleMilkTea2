...
public abstract class EdibleEntityItemBlock extends ItemBlock implements IEdibleItem {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        if (effect != null && this.showTooltip) {
            for (PotionEffect p : effect) {
                String s = StatCollector.translateToLocal(p.getEffectName())
                    .trim();
                if (p.getAmplifier() > 0) {
                    if (p.getAmplifier() < 4) {
                        s = s + " "
                            + StatCollector.translateToLocal("potion.potency." + p.getAmplifier())
                                .trim();
                    } else {
                        s = s + " " + p.getAmplifier();
                    }
        ...
        if (item.stackSize == 0) {
            ...
            .isSolid()) {
                ...
                if (DCsConfig.allowEdibleEntities) {
                    ...
                    if (!world.isRemote) {
                        if (this
                            .spownEntityFoods(world, player, new ItemStack(this, 1, m), x + 0.5F, y + 0.0F, z + 0.5F)) {
                    ...
                    int j1 = this.field_150939_a
                        .onBlockPlaced(world, x, y, z, side, p_77648_8_, p_77648_9_, p_77648_10_, i1);
