package mods.defeatedcrow.common.tile.appliance;
import net.minecraft.world.entity.player.Player;
import net.minecraft.world.entity.player.Inventory;
import net.minecraft.world.inventory.AbstractContainerMenu;
import net.minecraft.world.inventory.Slot;
import net.minecraft.world.item.ItemStack;
import net.minecraft.world.inventory.ContainerData;
import net.minecraft.world.inventory.SimpleContainerData;
public class ContainerAdvProcessor extends AbstractContainerMenu {
    public final TileAdvProcessor tile;
    private final ContainerData data;
    public ContainerAdvProcessor(int id, Inventory inv){ this(id, inv, null); }
    public ContainerAdvProcessor(int id, Inventory inv, TileAdvProcessor t){
        super(mods.defeatedcrow.common.registry.ModMenuTypes.ADV_PROCESSOR.get(), id);
        this.tile=t;
        if(t!=null){
            this.data = new SimpleContainerData(2);
            this.data.set(0, t.cookTime);
            this.data.set(1, t.chargeAmount);
            addDataSlots(this.data);
        } else {
            this.data = new SimpleContainerData(2);
            addDataSlots(this.data);
        }
        if(t!=null){
            this.addSlot(new Slot(t, 0, 9, 9));
            for(int j=0;j<3;j++){
                for(int k=0;k<3;k++){
                    this.addSlot(new Slot(t, 2 + k + j*3, 33 + k*18, 16 + j*18));
                }
            }
            this.addSlot(new Slot(t, 11, 118, 43){ @Override public boolean mayPlace(ItemStack s){ return false; }});
            this.addSlot(new Slot(t, 12, 145, 43){ @Override public boolean mayPlace(ItemStack s){ return false; }});
            this.addSlot(new Slot(t, 13, 91, 16));
        } else {
            for(int i=0;i<13;i++) this.addSlot(new Slot(new net.minecraft.world.SimpleContainer(14), i, 33 + (i%3)*18, 16 + (i/3)*18));
        }
        for(int i=0;i<3;i++){
            for(int j=0;j<9;j++){
                this.addSlot(new Slot(inv, j + i*9 + 9, 8 + j*18, 84 + i*18));
            }
        }
        for(int i=0;i<9;i++){
            this.addSlot(new Slot(inv, i, 8 + i*18, 142));
        }
    }
    public int getCookTime(){ return data.get(0); }
    public int getCharge(){ return data.get(1); }
    @Override public boolean stillValid(Player p){
        if(tile==null) return false;
        return tile.stillValid(p) && p.distanceToSqr(tile.getBlockPos().getX()+0.5, tile.getBlockPos().getY()+0.5, tile.getBlockPos().getZ()+0.5) < 64;
    }
    @Override public ItemStack quickMoveStack(Player player, int index){
        ItemStack ret = ItemStack.EMPTY;
        Slot slot = this.slots.get(index);
        if(slot!=null && slot.hasItem()){
            ItemStack stack = slot.getItem();
            ret = stack.copy();
            int tileSize = 13;
            if(index < tileSize){
                if(!this.moveItemStackTo(stack, tileSize, this.slots.size(), true)) return ItemStack.EMPTY;
            } else {
                if(TileAdvProcessor.isItemFuel(stack)){
                    if(!this.moveItemStackTo(stack, 0, 1, false)) return ItemStack.EMPTY;
                } else {
                    if(!this.moveItemStackTo(stack, 1, 10, false)) return ItemStack.EMPTY;
                }
            }
            if(stack.isEmpty()) slot.set(ItemStack.EMPTY);
            else slot.setChanged();
            if(stack.getCount()==ret.getCount()) return ItemStack.EMPTY;
            slot.onTake(player, stack);
        }
        return ret;
    }
    @Override public void broadcastChanges(){
        super.broadcastChanges();
        if(tile!=null){
            data.set(0, tile.cookTime);
            data.set(1, tile.chargeAmount);
        }
    }
}
