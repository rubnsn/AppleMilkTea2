package mods.defeatedcrow.common.tile.appliance;
import net.minecraft.world.entity.player.Player;
import net.minecraft.world.entity.player.Inventory;
import net.minecraft.world.inventory.AbstractContainerMenu;
import net.minecraft.world.inventory.Slot;
import net.minecraft.world.item.ItemStack;
import net.minecraft.world.inventory.ContainerData;
import net.minecraft.world.inventory.SimpleContainerData;
public class ContainerEvaporator extends AbstractContainerMenu {
    public final TileEvaporator tile;
    private final ContainerData data;
    public ContainerEvaporator(int id, Inventory inv){ this(id, inv, null); }
    public ContainerEvaporator(int id, Inventory inv, TileEvaporator t){
        super(mods.defeatedcrow.common.registry.ModMenuTypes.EVAPORATOR.get(), id);
        this.tile=t;
        if(t!=null){
            this.data = new SimpleContainerData(1);
            this.data.set(0, t.cookTime);
            addDataSlots(this.data);
        } else {
            this.data = new SimpleContainerData(1);
            addDataSlots(this.data);
        }
        if(t!=null){
            // map 3-slot tile to original 6-slot layout simplified: 0 fuel 9,9 ; 1 input 56,17 ; 2 output 56,55 ; extra slots dummy
            this.addSlot(new Slot(t, 0, 56, 17));
            this.addSlot(new Slot(t, 1, 56, 55){ @Override public boolean mayPlace(ItemStack s){ return false; }});
            this.addSlot(new Slot(t, 2, 141, 58));
        } else {
            for(int i=0;i<3;i++) this.addSlot(new Slot(new net.minecraft.world.SimpleContainer(3), i, 56, 17+i*18));
        }
        for(int i=0;i<3;i++){
            for(int j=0;j<9;j++){
                this.addSlot(new Slot(inv, j + i*9 + 9, 8 + j*18, 84 + i*18));
            }
        }
        for(int i=0;i<9;i++){
            this.addSlot(new Slot(inv, i, 8 + i*18, 142));
        }
    }
    public int getCookTime(){ return data.get(0); }
    @Override public boolean stillValid(Player p){
        if(tile==null) return false;
        return tile.stillValid(p) && p.distanceToSqr(tile.getBlockPos().getX()+0.5, tile.getBlockPos().getY()+0.5, tile.getBlockPos().getZ()+0.5) < 64;
    }
    @Override public ItemStack quickMoveStack(Player player, int index){
        ItemStack ret = ItemStack.EMPTY;
        Slot slot = this.slots.get(index);
        if(slot!=null && slot.hasItem()){
            ItemStack stack = slot.getItem();
            ret = stack.copy();
            int tileSize = 3;
            if(index < tileSize){
                if(!this.moveItemStackTo(stack, tileSize, this.slots.size(), true)) return ItemStack.EMPTY;
            } else {
                if(!this.moveItemStackTo(stack, 0, 1, false)) return ItemStack.EMPTY;
            }
            if(stack.isEmpty()) slot.set(ItemStack.EMPTY);
            else slot.setChanged();
            if(stack.getCount()==ret.getCount()) return ItemStack.EMPTY;
            slot.onTake(player, stack);
        }
        return ret;
    }
    @Override public void broadcastChanges(){
        super.broadcastChanges();
        if(tile!=null) data.set(0, tile.cookTime);
    }
}
