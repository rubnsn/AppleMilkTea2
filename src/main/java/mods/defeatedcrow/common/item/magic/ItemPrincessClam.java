                    ...
                    && par3World.isSideSolid(posX, posY, posZ, ForgeDirection.UP))// 2ブロックの空間がある
                {
                    ...
                    if (flag) {
                        ...
                        par2EntityPlayer.addChatMessage(
                            new ChatComponentText("Registered warp pos : " + posX + ", " + (posY + 1) + ", " + posZ));
                        return true;
                    }
                ...
                if (mode == 1)// playerモード
                {
                    ...
                    if (target != null) {
                        ...
                        if (flag) {
                            ...
                            boolean sameDim = target.worldObj.provider.getDimensionName()
                                .equalsIgnoreCase(world.provider.getDimensionName());
                            ...
                            if (thisWorld && sameDim) {
                                ...
                                thisPlayer.addChatMessage(
                                    new ChatComponentText("Fail to get position of the registered player."));
                            }
                        } else {
                            thisPlayer.addChatMessage(
                                new ChatComponentText(
                        ...
                        thisPlayer
                            .addChatMessage(new ChatComponentText("Fail to get position of the registered player."));
                    }
                }
            } else if (meta == 4)// moon
            {
                ...
                for (int i = 1; i < 128; i++) {
                    ...
                    if (this.moonCanWarp(world, X, Y + i, Z) && world.isAirBlock(X, Y + i + 2, Z)) {
                        if (world.isAirBlock(X, Y + i + 1, Z)) {
                            ...
                            flag = true;
                            break;
                        } else if (world.getBlock(X, Y + i + 1, Z)
                            .getMaterial() == Material.water
                            || world.getBlock(X, Y + i + 1, Z)
            ...
            if (entity instanceof EntityPlayer && entity.isEntityAlive()) {
                ...
                if (targetItem.getItem() == this) {
                    ...
                    target.addChatMessage(
                        new ChatComponentText(player.getDisplayName() + " registered you to the Raden Charm"));
                }
