...
/**
 * Fondue: ingredient -> result
 * JSON: { "type":"defeatedcrow:fondue", "ingredient":{...}, "result":{...} }
 */
public class FondueRecipe extends AMTRecipeBase {
    private final Ingredient ingredient;
    private final ItemStack result;
    public FondueRecipe(ResourceLocation id, Ingredient ingredient, ItemStack result) { super(id); this.ingredient=ingredient; this.result=result; }
    ...
    @Override public RecipeSerializer<?> getSerializer(){ return ModRecipes.FONDUE_SERIALIZER.get(); }
    @Override public RecipeType<?> getType(){ return ModRecipes.FONDUE_TYPE.get(); }
    public static class Serializer implements RecipeSerializer<FondueRecipe>{
        @Override public FondueRecipe fromJson(ResourceLocation id, JsonObject json){
            Ingredient ing=AMTRecipeBase.ingredientFromJson(json, "ingredient");
            ItemStack res=AMTRecipeBase.resultFromJson(json);
            return new FondueRecipe(id, ing, res);
        }
        @Override public FondueRecipe fromNetwork(ResourceLocation id, FriendlyByteBuf buf){
            Ingredient ing=AMTRecipeBase.readIngredient(buf);
            ItemStack res=AMTRecipeBase.readItemStack(buf);
            return new FondueRecipe(id, ing, res);
        }
        @Override public void toNetwork(FriendlyByteBuf buf, FondueRecipe r){
